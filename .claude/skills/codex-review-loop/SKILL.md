...
**프리플라이트(수정 커밋 전 1회)**: 로컬 체크아웃이 PR `<N>` 의 head 브랜치와 일치하는지 확인한다 — `gh pr view <N> --json headRefName` 과 `git branch --show-current` 를 대조. 불일치면(사용자가 `dev`·다른 브랜치에 있는 상태로 독립 호출한 경우) **커밋하지 말고 멈춰** 사용자에게 알린다 — 엉뚱한 브랜치에 수정이 꽂히면 안 된다. 자동 checkout 은 하지 않는다(이 저장소는 워크트리가 브랜치를 점유할 수 있어 checkout 이 실패·충돌할 수 있다). pr-cycle 경유 호출은 Phase 0–2 가 이미 PR 브랜치를 보장하므로 이 프리플라이트는 통과한다.
