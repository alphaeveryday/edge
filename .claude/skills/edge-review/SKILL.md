...
- **변경 영역을 판정**해 아래 조건부 각도를 켠다: schema(`libs/schema`)? · gateway/`*-api`(JVM 신뢰경계)? · `data-pipeline`/`analysis-engine`(Python 레이크)? · UI(`*-ui`/`ui-kit`)? · 전역 설정/CI? · **IaC 또는 새 클라우드 API 호출 추가**(`infra/`·terraform `.tf`/`.tftpl`, 또는 앱이 boto3·AWS SDK 로 새 API 를 부르기 시작 — I각도. `.tf` 를 안 건드려도 권한 충분성은 봐야 한다)? · **프론트 서빙·패키징**(UI Dockerfile·정적 호스팅 conf·`VITE_*` 빌드 플래그·서빙되는 `dist` — J각도)? · **검증·정규화·파싱·품질 게이트 코드**(`validate_*`·`check_*`·`normalize*`·타입 강제·게이트 — 언어 무관)? · **문서화된 실행 경로**(README·runbook 에 추가·변경된 실행 명령·env 예시, 새 엔트리포인트·설정 표면 — K각도)? (L각도는 조건부가 아니다 — 항상 각도라 판정 없이 늘 들어간다.)
...
- ⚠️ **exit 코드만으로는 부족하다 — 산출물의 존재를 함께 본다.** 한도 소진은 `ERROR: You've hit your usage limit … try again at <날짜>` 를 stdout 에 찍고도 **exit 0 으로 끝난다**(2026-08-06 실측). 위 규칙만 지키면 리뷰가 없었는데 "패스 성공"으로 세어진다. 판정은 **`-o` 파일이 존재하고 비어 있지 않으며 JSON 으로 파싱되는가**로 한다 — 셋 중 하나라도 아니면 그 패스는 실패다(exit 0 이어도). 한도 소진이면 재시도가 무의미하니 즉시 중단하고, 재개 시각과 함께 사용자에게 알린 뒤 대체 게이트로 넘어간다.
...
- **D. 거버넌스·규칙(AGENTS 12룰)** — **명백한 위반만**, 규칙을 번호·문구로 인용해 flag. 특히:
  ...
  - **Rule 9 tests-encode-WHY** — 테스트가 WHAT만 검사하고 WHY를 인코딩하지 못하는가. 비즈니스 로직이 바뀌어도 못 깨지는 테스트는 잘못됐다. **단언 강도도 대조하라**: 테스트가 지키려는 계약의 **반례를 현재 단언이 실제로 거부하는지** 본다 — 단언이 계약보다 약하면 지키려는 회귀를 통과시킨다(ALPHA-599 실증: "result **필드 부재**" 계약을 `jsonPath().doesNotExist()` 로 단언했는데 이는 `"result": null` 명시도 통과 — 부재는 키 부재 단언(`not(hasKey())`·키 집합 전수 비교)이어야 했고, 로컬이 놓쳐 봇이 잡음). 대표 함정: `doesNotExist()`(null 통과)·존재 단언만으로 여분 키 방치·`contains`(순서·중복 무시)·부분 매칭.
...
- **L. 실패 전파·되먹임** (항상 켠다 — 조건 트리거로 걸면 새 호출 삽입 없이 실패 경로만 바뀌는 변경(기존 어댑터가 partial 을 방출하게 되는 수정·오케스트레이터 실패 전이 수정)에서 꺼진 채 지나간다. 세 질문뿐이라 늘 켜는 게 싸다) — A~K 는 전부 *변경된 코드 안*이 틀렸는지 묻는다. 이 각도는 **코드가 옳아도 나는 사고**를 묻는다. 셋 다 답이 diff 안이 아니라 **호출 순서·오케스트레이터 정의·어댑터 내부**(레포 안의 미변경 파일)에 있어서, diff 만 읽으면 구조적으로 안 보인다:
  - **① 이게 죽으면 뒤가 도는가** — 이 변경으로 **'실패 가능 지점 → 하류 작업' 관계가 생기거나 달라졌으면**(앞에 새 코드 삽입·기존 선행 코드의 실패 거동 변경·기존 실패 경로 **뒤에 새 작업 추가**·순서 재배열 — 어느 쪽이 바뀌었든) 그 실패가 무엇을 막는지 세어라. 보조 판정(캐시 조회·사전 스캔·검증)이 본 작업 앞에서 예외를 올리면 본 작업이 통째로 안 돈다. 특히 그 실패가 **다음 런의 입력을 그대로 두어 자기증폭**하는지: 실패 → 산출물 없음 → 입력 불변 → 다음 런도 같은 자리에서 실패 = 수동 복구 전까지 영구 정지. 실패 처리 방향은 예외 유형이 아니라 **뒤 작업이 그 읽기 결과에 의존하는가**로 갈라라: 보조 읽기(캐시·힌트)의 인프라 실패는 **격리**해 본 작업을 살리고, 뒤 작업의 전제를 세우는 읽기(완전성 검증·정합 판정)의 실패는 격리하지 말고 **명시적 partial/error 로 드러내라** — 격리한 채 계속 가면 모르는 상태로 성공을 보고한다(Rule 12). 입력 오류(잘못된 인자)는 어느 쪽이든 그대로 올려라 — 뒤 작업이 같은 값을 쓰면 삼키는 순간 틀린 채로 성공한다.
  - **② 이번 런의 출력이 다음 런의 판정 입력인가** — 상태를 읽고 쓰는 것이 같은 파이프라인이면 판정이 **자기 출력에 오염된다**. "정본에 그 키가 있다"로 완전성을 판정하면, 이번 런이 불완전하게 쓴 행이 다음 런에 "이미 있음"이 되어 재시도 자격이 영영 사라진다. 존재가 아니라 **깊이·버전·명시 상태**로 판정하는지 확인하라. 불완전하게 들어오는 문을 전부 세라(부분 실패분 저장·어댑터가 실패와 함께 방출하는 행·페이지 상한 절단).
  - **③ 오케스트레이터가 실패를 하류로 보내는가** — "실패했으니 안 들어갔겠지"를 **정의 파일을 열어** 확인하라. 이 레포는 `statemachine.tf` `NotifyRawPartial` 이 *"raw 부분 실패는 뒤 페이즈를 막지 않는다"*(ALPHA-460)라 **partial 런의 부분 출력이 정제를 거쳐 정본에 들어간다**. 그래서 "partial 로 보고했으니 드러났다"가 거짓일 수 있다 — 보고는 됐는데 결손은 영구인 형태.

  ALPHA-989 실증: 로컬 게이트 6라운드(수용 10건)가 전부 "판정이 틀리는가"만 물어 통과시켰고, 봇이 위 셋으로 **P1 8건**을 냈다(수집 전체 정지·자기 오염·partial 통과). 셋 다 diff 밖 사실이 근거였다.

**조건부 각도 (Phase 0 이 켠 것만 지시문에 넣는다)**
- **E. 스키마 계약** (`libs/schema` 변경 **또는 DB 쓰기 코드**(리포지토리·`persist`·마트 적재 등) 변경 시) — 스키마 변경에 `generated` 모델 동반 갱신이 빠졌는가(docs/git-conventions.md 원칙). 마이그레이션이 expand-contract(파괴적 DDL을 한 번에)를 어겼는가. **단일 writer 위반**([ADR-0005](../../../docs/adr/0005-db-as-contract.md)·[docs/implementation.md](../../../docs/implementation.md) §4) — 한 테이블을 소유 모듈 밖에서 INSERT/UPDATE/DELETE 하는가. 이 위반은 스키마 diff 가 아니라 **앱 DB 접근 코드**에서 터지므로, 스키마 변경이 없어도 DB 쓰기 코드가 바뀌면 검사한다.
...
- **I. IaC 교체·상태 안전성 / 권한 충분성** (terraform·`infra/` 변경 **또는 앱이 새 클라우드 API 호출을 추가할 때** — `.tf` 를 안 건드려도 켠다) — 인프라 변경이 **의도 밖의 리소스 교체·상태 손실**을 부르는지, 그리고 **부여된 권한이 실제 호출을 덮는지**. 코드가 보통 넘어가는 이 함정들을 적대적으로 확인한다:
  ...
  - **부여한 권한 ↔ 소비자가 실제 호출하는 API** — IAM 정책(또는 SG·역할 신뢰관계)을 만들거나 넓힐 때, 그리고 **앱이 새 클라우드 API 호출을 추가할 때**, 그 권한을 쓰는 **애플리케이션 코드를 열어 어떤 API 를 호출하는지 세고** 액션 집합이 그걸 덮는지 대조한다. `.tf` 안만 보면 문법상 멀쩡해 통과한다. 특히 **호출부가 예외를 폴백으로 처리하면 AccessDenied 가 실패로 안 보인다** — 리소스는 생겼고 apply 도 초록이고 런도 성공으로 끝나는데 의도한 경로만 영영 안 도는 형태다. 폴백이 사유를 기록하더라도 **그것을 출력하는 경로가 진단 CLI 에만 있고 운영 소비자 경로에 없으면** 결과는 같다(ALPHA-671 실증: Glue Iceberg 읽기 정책이 database/table/partition 액션만 줘 `glue:GetCatalog` 가 빠졌는데, `duck.py` 의 `_bars_iceberg()` 가 attach 예외를 `unbound` 에 담고 `False` 를 반환하면 호출부가 곧장 non-Glue 폴백으로 내려간다. 그 `unbound` 는 `Lake.coverage()` 가 출력하지만 호출자가 스모크 CLI 뿐이라 운영 런에서는 안 보인다. 로컬 리뷰가 `.tf` 안만 보고 놓쳐 봇이 P1 로 잡음 — 봇은 소비자 코드와 벤더 문서를 함께 읽었다).
...
- **K. 문서화된 실행 경로의 실행 가능성** (diff 가 **문서에 실행 명령·env 예시를 추가·변경**할 때 — README·runbook 의 `python -m …`·CLI 호출·`VAR=… command` 블록, 새 엔트리포인트·설정 표면 도입 시 특히) — "문서가 코드와 맞는가"(그건 docs-sync 소관)가 아니라 **그 명령을 그대로 쳤을 때 되는가**를 본다. 새 실행 표면의 첫 문서 경로가 죽어 있으면 배포·인수인계 시점에야 드러난다. 문서에 적힌 형태를 **코드가 실제로 받는 형태와 대조**하라:
  ...
  - **문법 검사는 확인이 아니다** — `bash -n` 은 실행 없이 읽기만 해 이 각도가 잡으려는 유형을 통과시킨다(실측: 하이픈 든 env `A__b-c=v true`·없는 모듈 `python -m x.y` 둘 다 exit 0). 확인하려면 그 줄이 실제로 한 번 돌아야 한다: prefix env 는 `VAR=… true`, 명령은 `--help`/`--dry-run`/import-only/`plan` 같은 **읽기 전용 경로**.
  - ⚠️ **상태를 바꾸는 명령(`terraform apply`·배포·마이그레이션·쓰기 CLI)은 돌리지 마라** — 리뷰가 실제 인프라를 바꾸면 안 된다(레포에 한 줄짜리 apply runbook 이 있다). 읽기 전용 경로가 없으면 형태만 보고 후보로 낸다. 외부 상태를 바꾸는 명령(`terraform apply`·배포·마이그레이션·쓰기 CLI)은 **돌리지 마라**: 리뷰가 실제 인프라를 바꾸면 안 된다(레포에 한 줄짜리 apply runbook 이 있다). 형태만 보고 후보로 낸다. 단 **게이트 뒤에 오는 docs-sync 가 새로 쓰는 실행 예시는 이 각도가 볼 수 없다**(pr-cycle 은 edge-review 루프를 먼저 끝내고 docs-sync 를 1회 돌린다) — 그쪽은 docs-sync 가 자기 수정분을 검증한다. ALPHA-670 실증: 새 relay 엔트리포인트의 README 예시가 `DATA_PIPELINE_…__QUEUE_URLS__price-analysis-realtime=` 형태라 셸이 못 읽어 첫 문서 실행 경로가 통째로 불가였는데, 로컬 게이트 7라운드(수용 46건)가 코드만 보느라 놓치고 봇이 잡음.
**검증 (같은 실행 안에서)**
