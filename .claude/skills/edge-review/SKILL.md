...
- **L. 실패 전파·되먹임** (변경이 **기존 호출 경로에 코드를 끼워 넣거나**, 파이프라인이 **읽는 저장소에 그 파이프라인이 쓰기도** 할 때. 항상 켜도 싸다 — 세 질문뿐이다) — A~K 는 전부 *변경된 코드 안*이 틀렸는지 묻는다. 이 각도는 **코드가 옳아도 나는 사고**를 묻는다. 셋 다 답이 레포 안이 아니라 **호출 순서·오케스트레이터 정의·어댑터 내부**에 있어서, diff 만 읽으면 구조적으로 안 보인다:
  - **① 이게 죽으면 뒤가 도는가** — 새 코드가 기존 호출 **앞**에 놓였으면 그 실패가 무엇을 막는지 세어라. 보조 판정(캐시 조회·사전 스캔·검증)이 본 작업 앞에서 예외를 올리면 본 작업이 통째로 안 돈다. 특히 그 실패가 **다음 런의 입력을 그대로 두어 자기증폭**하는지: 실패 → 산출물 없음 → 입력 불변 → 다음 런도 같은 자리에서 실패 = 수동 복구 전까지 영구 정지. 읽기 실패(인프라·손상)는 **격리**하고 입력 오류(잘못된 인자)는 **그대로 올려라** — 뒤 작업이 같은 값을 쓰면 삼키는 순간 틀린 채로 성공한다.
