...
- **L. 실패 전파·되먹임** (항상 켠다 — 조건 트리거로 걸면 새 호출 삽입 없이 실패 경로만 바뀌는 변경(기존 어댑터가 partial 을 방출하게 되는 수정·오케스트레이터 실패 전이 수정)에서 꺼진 채 지나간다. 세 질문뿐이라 늘 켜는 게 싸다) — A~K 는 전부 *변경된 코드 안*이 틀렸는지 묻는다. 이 각도는 **코드가 옳아도 나는 사고**를 묻는다. 셋 다 답이 레포 안이 아니라 **호출 순서·오케스트레이터 정의·어댑터 내부**에 있어서, diff 만 읽으면 구조적으로 안 보인다:
