...
1. **edge-review 수렴 루프** — 한 라운드는: edge-review 스킬 실행 → finding 수용 판단(기준은 codex-review-loop 의 "수용 판단"과 동일 — 실질 여부·의도적 생략) → 수용 finding 을 이 PR 안에서 수정. 수용 finding 이 하나라도 있었으면 **전체 범위를 다시 리뷰**한다 — 수정분이 재리뷰 없이 push 되어 새 finding 을 낳는 것이 봇 왕복이 길어지는 주범이다. **한 라운드의 수용 finding 이 0건이면 수렴.** 상한 3라운드 — 미수렴이면 PR 은 진행하되 잔여 finding 과 미수렴 사실을 PR 본문·머지 보고에 명시한다(Rule 12, 봇이 백스톱).
   - effort 는 이 게이트가 지정한다: 기본 `high`, 넓거나 위험한 변경(신규 모듈·전면 재구축·`libs/schema`·신뢰경계)은 `max`. edge-review 단독 호출의 기본값(medium)과 별개다 — 봇 1회 리뷰와 대등한 깊이를 매 라운드 확보해야 봇 왕복이 1라운드로 줄어든다.
   - edge-review 는 변경 모듈의 **빌드/테스트를 함께 확인**해 실패를 최우선 finding 으로 올리므로 매 라운드 검증된다(별도 빌드/테스트 단계를 두지 않는 이유 — 중복). 마지막 라운드 통과가 곧 게이트 통과다.
2. **docs-sync 스킬 실행** — 루프 **수렴 후 1회**. 코드가 바꾼 사실이 문서에 반영됐는지 점검하고 드리프트를 이 PR 안에서 함께 해소한다. 사후 "문서 정합성 정정" 커밋이 반복돼 온 이력이 있다. (루프가 코드를 계속 바꾸는 동안 점검하면 헛돈다 — 수렴 뒤에 둔다.)
...
- 4단계 수렴 루프를 거쳤으므로 기대치는 **1라운드 내 `+1`** 이다. 봇 왕복이 2라운드를 넘으면 로컬 게이트가 못 잡는 유형이 있다는 신호다 — 사이클 종료 후 그 finding 유형으로 edge-review 각도 보강을 제안하라. 봇이 잡고 로컬이 놓친 finding 이 게이트 개선의 입력이다(H각도 신설이 그 예).
