...
**풀링** — 풀링을 시작하기 전(최초 생성 직후·재리뷰 요청 후·PR 보완 모드 재진입 모두) 현재 리액션·리뷰 상태를 베이스라인으로 기억하고, **그 이후 생긴 신호만** 유효로 판정한다. 리액션·리뷰는 PR에 붙지 커밋에 붙지 않아서, 이전 라운드의 `+1`이나 기존 리뷰를 새 응답으로 오인하면 새 커밋이 무리뷰로 통과되거나 이미 반영한 지적을 다시 돈다. 60초 간격으로 두 신호를 확인한다 (보통 수 분 내 응답, 10분 넘게 무응답이면 사용자에게 보고하고 대기 여부를 확인):
...
**반영 루프** — 하나라도 수용했다면: 수정 커밋 → push → PR에 `@codex review` 코멘트로 재리뷰 요청 → 베이스라인 갱신 후 다시 풀링. 새 `+1`이 달리거나, 남은 finding이 전부 비수용 판정이면 루프를 끝내고 머지로 진행한다.
