...
| 2026-07-22 | PR 전 게이트를 로컬 수렴 루프로 개편 — edge-review(게이트 기본 `high`·위험 변경 `max`)를 수용 finding 0건까지 전체 범위 재리뷰로 반복(상한 3라운드, 미수렴 시 명시 후 진행), docs-sync는 수렴 후 1회, Phase 6 봇 왕복 기대치 1라운드 + 초과 시 각도 보강 제안 신설 | skills/pr-cycle | 실측(최근 10개 PR): 봇 finding 대부분이 로컬에서 잡히는 성격(#191 5라운드·8건, E·G·C각도 해당)인데 게이트가 medium 1회 실행이라 커버리지 부족 + 수정분 무재리뷰가 왕복 장기화(작업당 최대 ~1시간)의 주범 — GitHub 왕복을 대기 없는 로컬 라운드로 흡수 |
