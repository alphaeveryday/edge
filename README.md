...
<!-- TODO(이미지): MTS 데모 실기기 스크린샷 2~3장 + 데모 QR/링크, 테넌트 콘솔·슈퍼 어드민 콘솔 스크린샷 각 1장 -->
...
콘솔의 화면과 기능을 트리 하나로 정리한 화면 맵입니다. 왼쪽이 증권사 검수자가 쓰는 테넌트 콘솔(대시보드·설명 관리·검수·점검 기준·설정), 오른쪽이 운영자가 쓰는 슈퍼 어드민 콘솔(테넌트 관리·분석 관측)입니다. 설계 시점(v0.2) 그림이라 이후 폐지한 기능(정정 처리·위험 등급)이 일부 남아 있습니다.
...
전체 시스템을 Service·Worker·Cache·DB 논리 컴포넌트 단위로 나타낸 구성도입니다. 왼쪽 증권사 통제 환경(위젯·콘솔·서빙·검수 워커)과 오른쪽 EDGE 환경(수집·분석 워커·동기화)은 동기화 경로 하나로만 이어집니다. 이 그림도 설계 시점 기준이라 이후 제거하기로 한 Gateway가 남아 있고, BYOC 기준 재작도를 앞두고 있습니다.

### 2-4. Cloud Architecture

![클라우드 아키텍처 — AWS 배치(VPC·서브넷·ECS 클러스터)](docs/assets/readme/CA.drawio.png)

논리 컴포넌트를 실제 AWS 리소스로 배치한 인프라 구성도입니다. 서브넷을 public/앱/데이터로 분리하고 가용영역을 이중화했으며, 전체를 Terraform으로 관리합니다. 위 그림은 설계 시점의 단일 계정 배치 기준이라 이후 결정이 반영되지 않은 부분(ElastiCache — 실측 후 도입 보류)이 있고, BYOC 2계정 구도로 재작도할 예정입니다.
