...
지금 결정해야 하는 이유는 **엔터티 마스터 시딩(ALPHA-362)이 첫 값을 넣는 순간 그 포맷이 사실상 계약이 되기 때문**이다. 이 마스터 ID들은 스키마 전역에서 **21개 FK**가 참조한다(`instrument` 10 · `entity` 7 · `actor` 2 · `company_profile` 2). 한 번 적재된 뒤 포맷을 바꾸려면 참조 전부를 마이그레이션해야 하고, assertion·event는 point-in-time 재현 대상이라 소급 수정이 특히 비싸다. 형제 사례가 이미 있다 — `document_assertion.modality_code`는 어휘가 정의된 적 없는 채 `NOT NULL`이라 적재를 통째로 막았고 ALPHA-361로 제약을 되물러야 했다. 값보다 규약을 먼저 박는다.
