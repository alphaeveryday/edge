...
- 대체: [ADR-0044](0044-correction-abolition.md) 중 "Cloud 운영자 정정 오버레이 유지" 결정
  부분 — 콘솔 통제는 무효화 단독으로 교체됐다(ALPHA-737, 사용자 확정 2026-08-03). 감사
  원장(admin_activity_log)·과거 기록은 존치.
...
- **콘솔 통제 교체(ALPHA-737)**: 무효화가 실 통제(WITHDRAWN + 테넌트 전파)로 구현되면서
  구 콘솔 오버레이 3종(정정·제외·복원 — 테넌트 미전파 표시 전용)은 은퇴했다. 0044 결정 3의
  "오버레이 유지"는 이 부분에서 대체된다(헤더 대체 표기).
