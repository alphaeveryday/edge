...
- **CD(지속적 배포)** — dev 는 `.github/workflows/deploy-dev.yml` 로 구현됐다: dev push 마다 스키마 migrate 를 먼저 실행하고, path 필터로 바뀐 앱만 독립 배포한다(`needs: migrate`). 원칙은 그대로다 — "전체 일괄 자동 배포"는 두지 않고, 마이그레이션 확장 단계가 코드 배포보다 먼저다. prod 앱 배포는 prod 인프라 확정 후 같은 구조로 잇는다.
