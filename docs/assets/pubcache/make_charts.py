# publication-api 캐시 실험 실측 차트 3종 (PNG, light)
...
MODES = [('none', 'none\n(캐시 없음)', C_NONE), ('caffeine', 'caffeine\n(L1)', C_CAF),
         ('redis', 'redis\n(L2)', C_RED), ('two-level', 'two-level\n(L1+L2)', C_TWO)]
...
fig.suptitle('고정 부하에서 캐시 모드별 응답 지연 — 전 모드 1~2ms, 지연 축 차이 없음',
...
fig.savefig(os.path.join(SP, '1-mode-latency-p95-p99.png'), bbox_inches='tight')
plt.close(fig)

# ============ 그림 2: 워킹셋 스윕 p99 ============
Ns = [100, 300, 800, 1088, 3000, 5000]
fig, ax = plt.subplots(figsize=(10, 5.2), dpi=200)
for mode, color, name in [('caffeine', C_CAF, 'caffeine'), ('two-level', C_TWO, 'two-level')]:
    meds, xs = [], []
    for n in Ns:
...
style_ax(ax)
ax.set_title('워킹셋 크기 스윕 — L1 임계점을 지나면 two-level의 L2 왕복 비용이 꼬리를 키운다',
...
fig.tight_layout()
fig.savefig(os.path.join(SP, '2-working-set-sweep-p99.png'), bbox_inches='tight')
plt.close(fig)
...
for i, (mode, _, color) in enumerate(rows):
...
ax.set_title('캐시의 DB 오프로딩 — 지연이 아니라 DB 부하 축에서 갈린다',
             loc='left', fontsize=14.5, fontweight='bold', color=INK, pad=26)
ax.text(0, 1.06, 'hot-key 3키 · 1,600 rps · API 4대 · 3분 × 3회  (큰 점=중앙값, 반투명 점=반복 run · redis r1은 재기동 콜드 오염)',
        transform=ax.transAxes, fontsize=10, color=INK2)
fig.tight_layout()
fig.savefig(os.path.join(SP, '3-db-offloading.png'), bbox_inches='tight')
plt.close(fig)
print('done')
