...
- 처리 기준: 자동 노출 기준(출처 수·확신도 — 화면 라벨은 축만 지고 조건은 값이 진다: "2개 이상"·"중간 이상"(ALPHA-755). 원인 미확인(UNCERTAIN)은 기준과 무관하게 상시 검수, [../adr/0046](../adr/0046-confidence-gate-risk-grade-abolition.md)), 검수 필요 기준, 차단 기준, 기본 안내 문구. **온보딩 기본은 자동 노출 ON — 점검(금칙어·기준)에 걸린 것만 검수한다**(2026-07-27 결정, ALPHA-438). 전건 검수(0%) 운영은 테넌트 선택지로 남긴다 — 보수적 증권사는 실계약 시 자동 제공 스위치를 끄고 시작할 수 있다(policy_version.auto_publish_enabled)
