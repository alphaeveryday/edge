...
> **적용 시점**: 2026-07 아키텍처 피벗 이후. 이 문서와 충돌하는 기존 문서(Cloud-only / **벤더 서빙형** embed widget 구조)는 전부 폐기된 방향이다. (위젯 UI 자체는 빌드 산출물로 납품 — [adr/0035](adr/0035-widget-ui-build-artifact.md).)
...
**한 줄 정의**: EDGE는 "AI 리포트 생성기"가 아니라, **증권사가 통제 가능한 'ETF 가격 변동 이유 설명' 운영 인프라**다.
...
1. 고객 데이터 비반출 — 고객 데이터를 벤더 클라우드로 내보내지 않는다(ADR-0053 이후로는 받지도 않는다 — 고객 식별 비수취).
...
**MVP 제공 기능**: ETF별 가격 변동 이벤트 탐지 / 뉴스·공시·시세·수급 기반 변동 요인 후보 생성 / AI 설명 후보 생성 / 증권사별 컴플라이언스 정책 적용 / 자동 노출·검수 대기·차단 라우팅 / 검수자 승인·수정 승인·반려·차단 (고객 노출 이력·민원 재현은 [ADR-0053](adr/0053-widget-direct-serving-no-personalization.md)으로 기능 목록에서 제외 — 재구성은 게시·정정·정책 이력 수준)
