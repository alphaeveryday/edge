...
- ✅ **실행 상세로 가는 링크 복원 — 해소됨**(축 E). 실 API 화면 셋(실행 이력 `/grid` · 현재
  ...
  있는가**다. 안 쥔 자리(`SourcesPage` 의 조사 문맥 없음 분기 · 구성종목 결손의 런 미지정
  분기)는 여전히 **목록으로도 안 보낸다** — 목록도 하루치라 그 창 밖 런키를 든 사용자를 다시
  "없다"로 데려간다.
  ...
  242건이고, 그 변이는 아래 표의 **배선 검사**가 잡는다.
  ...
  | **상세가 그 값을 쓰는가** — 배선 | 주소에서 읽은 `date` 식별자가 `useConsoleEvaluation` 인자로 들어가는지 | `pages/ops/investigation.test.ts`. 변이(`useConsoleEvaluation(date)` → `()`)로 FAIL 확인. 인자가 옵셔널이라 **tsc 는 그 변이를 통과한다** — 레포 안에서 이걸 잡는 건 이 검사뿐이다 |
  | **상세가 그 값을 쓰는가** — 값 | 기본 창 **밖의** 런을 `?date=` 로 열어 실제로 해소되는지 — 스텁이 그 하루에만 그 런을 준다 | `verify-run-page-split`. ⚠️ **레포에 없다** — 브라우저 하네스는 `.dev/console-redesign/harness/` 에 있고 `.gitignore` 가 `/.dev` 를 통째로 뺀다. clone·CI 에서는 안 돈다(위 배선 검사가 그 자리의 바닥이다) |
  ...
  *실데이터인데 링크가 없는* 경우라 아무도 안 말해 줘서 카드가 `날짜 미상` 을 세운다. 그리고
  ...
  ⚠️ **자동 가드가 없는 분기다**: `.tsx` 안이라 `node --test` 가 못 보고(그 테스트는 소스에
  `date` 키가 **적혀 있는지**까지만 안다 — 런타임에 `undefined` 로 풀리는 것은 못 잡는다),
  목·날짜부재를 밟는 하네스 케이스도 없다(그 분기만 밟자고 스텁·dev 서버 한 벌을 더 세우는
  값이 안 맞는다). **링크를 손보는 라운드에서 이 네 자리를 손으로 같이 봐라.**
