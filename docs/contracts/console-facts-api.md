...
⚠️ 오늘 값과 기준일을 **한 문장으로** 센다 — 나눠 세면 두 값이 다른 스냅샷에서 나와 편차율이
**존재한 적 없는 비교**가 된다.
...
| 휴장일 제외는 **합집합 뒤 한 번** | 각 소스 안에서 빼면 다른 소스가 같은 날짜를 되살린다 |
...
⚠️ `trading_date` 는 **거래일 달력이 아니다**(Planner 가 휴장일에도 채운다). 휴장 판정은 기대
작업의 `skip_reason='NON_TRADING_DAY'` 에 있고, 그 신호는 **KR 시장 레인에만** 붙는다 — 그래서
제외는 **날짜 단위**다. 런 단위로 상관시키면 휴장일에도 도는 뉴스·공시 런이 그 날짜를 되살린다.

오늘이 휴장이면 `o.pub`·`o.trig` 의 `base` 를 **안 준다**. 그날 0 은 실측이 맞지만 **비교할
