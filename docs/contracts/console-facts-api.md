...
- ✅ **`deadJobsByDate` — `MinuteFacts.deadJobsByDate: string[]` 로 올렸다**(A2). 같은 데이터셋의
  ...
  없다"는 실측이어야 한다.
  🔴 **그런데 지금 생산자는 그걸 못 지킨다**(A2 검증 라운드에서 실증). 어댑터가 `newsJobs` 를
  **세션을 순회하며** 읽는데 `newsJobs` 는 세션과 무관한 날짜 축 집계다 — 그날 뉴스 세션이
  없으면(`sessions: []` 이거나 가격 세션만) `newsJobs.dead: 3` 이 있어도 `deadJobsByDate: []` 가
  나가고 R19 는 `평가됨 · 위반 0` 을 낸다. **유실 3건이 통째로 안 보인다.**
  뿌리는 축이 아니라 **값**이다: 축은 `MinuteFacts` 로 올렸는데 `deadJobs` 값은 여전히 세션에
  매달려 있어, 세션이 없으면 값이 실릴 자리가 없다. 세션 배열에 가짜 세션을 만들어 때우면 안
  된다(없는 실체를 지목한다) — 날짜 축 값은 **데이터셋 키 맵**으로 세션과 나란히 서야 한다.
  배선하며 이 축을 실 응답에서 파생시킬 때 값도 같이 옮겨라.
...
빈 문자열 조각(`compose`) · 축 선언 부재(`Array.isArray(deadJobsByDate)`) · `0` 과 실측의 구분
...
(`outputs: [null]`·`stages: {}`·`deadJobsByDate: [{}]`) · 음수 카운트 · `scope`·`aws_status` 가
