...
| ❌ 계측이 없어 `못 돎`(`notRun: 'axis'`) | R08 · R11 · R15 · R16 (+ `queues` 미배선이면 R12, minute 미도착이면 R17~R19) |
| ❌ 소스 자체가 없다 | R10 · R13 |
...
- ✅ **사건 식별자(`vid`) — 해소됨. 단 합성 축의 조각 가드는 남았다**(ALPHA-738, `rules/evaluate.ts`).
  ...
  **날짜가 들어가면 안 된다** — 키가 매일 달라져 어떤 조치도 등록 못 한다), `scope` 는 *어느 실행
  ...
  🔴 **여기서 남은 것 — 합성 축의 조각 가드.** 실시간 대상은 `${dataset}/${sourceGroup}` 로
  **합성**되는데, 평가기의 빈 축 가드는 합성 **후** 문자열만 본다(`targetId === ''`). 실 응답이
  `source_group` 을 빈 문자열로 주면 `price_minute/` 라는 정상처럼 보이는 vid 가 나가고, 그
  날짜의 세션이 둘 다 비면 R17~R19 가 통째로 `못 돎` 이 된다. 배선할 때 **조각 단위 가드**를
  같이 넣어라(위치는 A2 에서 정한다). ⚠️ 이 가드는 **아직 아무 단언도 안 잡는다** —
  어댑터 테스트(`pages/ops/consoleFacts.test.ts`)가 그 사실을 주석으로 밝힌다.
- 🔶 **`canRun` 이 거짓인 *사유 문장* 이 없다.** 규칙은 `axis`(모자란 축)만 밝히고, 화면은 그 축의
  조회 상태(대기·실패)로 문장을 만든다. 축이 같은데 사유가 다른 규칙이 생기면(예: 같은 minute
  응답을 읽는데 하나는 필드 부재, 하나는 미도착) 두 규칙이 같은 문장을 받는다. A2("못 돎
  정직화")에서 `Rule` 에 사유 축을 넣어라 — 지금은 축 표기가 그 자리의 최소값이다.
- 🔶 **`deadJobsByDate` 가 데이터셋 축 사실인데 세션 필드에 앉아 있다**(`MinuteSessionFact`).
  오늘은 어댑터가 데이터셋 함수(`datasetKind`)로 균일하게 넣어 섞임이 불가능하지만, 그 보증이
  **생산자의 습관**에 걸려 있다. 축을 `MinuteFacts`(데이터셋 집합)로 올리면 같은 데이터셋의
  세션이 서로 다른 축을 갖는 상태가 **표현 불가**가 된다 — 배선하며 이 축을 실 응답에서
  파생시킬 때 같이 옮겨라.
