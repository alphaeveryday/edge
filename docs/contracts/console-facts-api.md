...
- ✅ **실행 상세로 가는 링크 복원 — 해소됨**(축 E). 실 API 화면 셋(실행 이력 `/grid` · 현재
  ...
  `tradingDate ?? 런 키의 슬롯 날짜` 이고, 서버의 조회 축은
  ...
  ⚠️ **`useConsoleEvaluation(date)` 는 실시간 축을 안 따라간다**(`/sources/minute` 에 날짜
  인자가 없다). 지난 날을 지목하면 R17~R19 는 오늘 세션 위에서 판정된다 — 사건 목록을 세우는
  화면은 날짜를 넘기면 안 되고, 지금 넘기는 곳은 런 하나를 여는 상세뿐이다.
  ...
  변이가 tsc·단위 240건·하네스 대부분을 **통과했다**(봇 5라운드가 실증).
  ...
  | 어느 날이 맞는 날인가 | `dateOfSlot` 의 **우선순위**(`tradingDate` 우선 · 없을 때만 런 키 · 못 읽으면 `null`) | `domains/sources/dailyRollup.test.ts`. 순수 함수라 갈리는 입력을 직접 넣는다 |
  ...
  | **상세가 그 값을 쓰는가** | 기본 창 **밖의** 런을 `?date=` 로 열어 실제로 해소되는지 — 스텁이 그 하루에만 그 런을 준다 | `verify-run-page-split`. 변이(`useConsoleEvaluation(date)` → `()`)로 FAIL 확인했다 |
