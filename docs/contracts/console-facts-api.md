...
- ⏭ **실행 상세로 가는 링크 복원 — 남았다(축 E).** ⚠️ **사유가 바뀌었다**: 그 화면은 이제 실
  원장을 읽는다. 남은 제약은 **창**이다 — 상세의 조회가 날짜 인자 없이 돌아 원장이 아는 **가장
  최근 날 하루**만 싣는데, 실 API 화면(실행 이력 `/grid` · 현재 실행 `/minute` · 구성종목 결손)은
  임의의 날짜를 다룬다. D 는 `RUN_DETAIL_UNAVAILABLE` 문구와 목적지 문장을 그 사실에 맞게
  고쳤을 뿐 링크는 여전히 끊겨 있다.
  **되살리는 조건**: `runHref` 가 그 화면의 거래일을 싣고 `RunDetailPage` 가
  `useConsoleFacts(date)` 로 그 날을 조회하면 이 상수가 통째로 사라진다.
  적용 기준은 "상세냐 목록이냐"가 아니라 **그 자리에서 사용자가 특정 런 identity 를 쥐고
  있는가**다. 안 쥔 자리(`SourcesPage` 의 조사 문맥 없음 분기 등)는 목록이 정확히 그 용도라
  대상이 아니다.
