...
> **기계가독 계약(JSON Schema)** — 번들 JSON 구조의 기계가독 실체는 [`libs/schema/contracts/event-bundle.schema.json`](../../src/libs/schema/contracts/event-bundle.schema.json)(draft 2020-12, ALPHA-497). 이 문서(시맨틱·필드 정의)가 상위 SSOT이고 schema.json 은 그 기계가독 층이다. producer(tenant-sync-api)·consumer(intake) 양단이 이 파일을 로드해 계약 테스트로 검증하며(`contract-test.yml` CI), tenant-sync-api `openapi.yaml` 도 이 파일을 `$ref` 한다.
