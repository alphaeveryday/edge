...
**형상** (`evidences` 는 조립 구현됨(ALPHA-718) — 근거 없는 런은 `[]`. `source_events` 는 타임라인 UI 도입 시 조립 — **현행은 항상 `[]`**. 기계가독 스키마([event-bundle.schema.json](../../src/libs/schema/contracts/event-bundle.schema.json))는 요소 형상을 정의하되 `minItems: 0`이라 빈 배열·populated 둘 다 수용, ALPHA-497):
