...
- **Cursor**: 테넌트별, Cloud 서버가 발번하는 단조 증가 sequence. 온프렘은 마지막 처리 cursor를 durable 저장하고 `?after={cursor}`로 Pull한다. Pull을 개시하는 주체는 DMZ의 **Sync Agent**이고, 권위 committed cursor의 durable 저장 주체는 2모듈 표준에서 **Intake(내부망)**·단일 모듈 옵션에서 sync-agent다([ADR-0036](../adr/0036-sync-agent-intake-topology.md), 조율 메커니즘은 [ADR-0052](../adr/0052-sync-two-module-standard-reaffirmed.md) 결정 5가 대체). 2모듈에서 Pull 재개점은 **Intake의 committed cursor가 유일한 권위**다 — sync-agent 는 durable 진행 상태를 갖지 않는 **무상태 릴레이**이고, Intake 가 매 Pull 의 `?after=` 로 자신의 committed cursor 를 전달한다. 번들 commit 실패 시 committed cursor 가 전진하지 않아 다음 틱이 같은 지점에서 재-Pull 하고 멱등 upsert 가 중복을 dedup 해 **유실 없이** 수렴한다 — 재개점이 Intake commit 을 앞설 수 있는 상태 자체가 없다(구 DMZ-local 마크·commit-ack 조율은 ADR-0052 로 대체).
...
- **전달 보장**: at-least-once. 중복·재-Pull(재시도 포함) 수신은 **무해해야 한다** — 요구사항은 ① 유실(skip) 없음 ② immutable raw 레코드 손상·충돌 없음이다.
