...
| ② | **KRX 업종지수 분봉** (장중 섹터층의 전제) | 현재 구간 모드는 "KRX 지수는 5분봉이 없다"는 이유로 섹터 ETF 대체(layers.py) — 지수 분봉이 서면 대체 자체가 불필요해질 수 있다. **KIS 업종지수 분봉 API 실재 여부가 선결 확인 항목** — 레포의 KIS 분봉 어댑터는 종목 TR(당일 FHKST03010200·소급 FHKST03010230)뿐이고 업종 TR 근거가 레포에 없다 [실측 필요] | 중 — API 실재 시 1분 레인 어댑터 재사용, 부재 시 대안 없음(섹터 ETF 대체 유지 확정) |
...
- **선결 확인** [실측 필요]: KIS 에 업종지수(코스피/코스닥 업종 코드) 분봉 TR 이 실재하는지.
  레포 근거 없음 — 확인 방법: KIS 개발자센터 TR 목록에서 업종/지수 분봉 조회 + dev 앱키로
  1콜 실측(업종 코드 예: 1013 전기전자). 확인 결과에 따라 두 갈래:
  - **실재 시**: 1분 레인의 봉만 받는 축(`sector_etf_ids` 와 같은 "판정 밖 참조 계열" 규약,
    ALPHA-842)으로 업종 코드를 편입. 어댑터는 `kis_minute` 벤더 무관부 재사용, 시장구분
    코드만 다를 가능성(iNAV 의 `"E"`↔`"J"` 사례처럼 TR·파라미터 실측 필수). 존은
    `canonical/market_data/sector_index_minute/…` (price_minute 동형, 결정적·불변 artifact
    키 + generation). 처리량: 업종 ~45콜/분 추가 — KIS 실측 14.8req/s 예산에서 3초 —
    수용 가능.
  - **부재 시**: 수집하지 않는다. 구간 모드 섹터층은 현행 섹터 ETF 대체(48종 1분 수집,
    ALPHA-842)를 **확정 설계**로 명문화하고 이 항목을 닫는다 — 지수 분봉을 다른 벤더로
    긁는 것(토스 등)은 업종지수 미제공이 확인된 범위에서 대안이 없다 [실측 필요:
    토스 API 의 지수 캔들 제공 여부].
- **주기·트리거·완전성**: 1분 레인 편입 시 세션 원장(window·manifest) 계약을 그대로 받는다
  — 별도 완전성 장치 불필요.
