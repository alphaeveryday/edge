...
**예외 — 소비자도 행도 없는 표**(ALPHA-853). 확장-수축은 **읽는 쪽이 있을 때** 옛 이름과 새 이름을 함께 두어 무중단 전환을 만드는 장치다. 정의만 있고 생산자·소비자·행이 전부 0 인 표에는 지킬 무중단이 없고, 없는 소비자를 위해 두 벌을 유지하면 다음 사람이 어느 쪽이 진짜인지 묻게 된다. 이때만 직접 `RENAME`·`DROP` 을 허용한다.

조건은 **넷 다** 충족해야 하며 근거를 PR 설명에 적는다:

- 코드 참조 0 — py·java·kt·sql·ORM 매핑·SQL 문자열 상수를 전수 grep
- **문서 참조 0** — 설계 문서가 그 표를 근거로 삼고 있으면 문서를 함께 정리하거나 예외를 포기한다. ALPHA-853 이 실제로 여기서 걸렸다(코드는 0 이었으나 활성 스펙이 그 표를 설계 근거로 인용하고 있었다)
- 행 0
- **되돌리기 경로를 마이그레이션 주석에 명시** — `DROP` 은 역이 없다. 원본 DDL 의 파일·줄 번호를 적는다
