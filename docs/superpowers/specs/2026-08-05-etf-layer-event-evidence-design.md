...
격자와 ATT가 같은 표본을 사용하는 현재 선택 편향은 번들의 `selection_caveat="same_sample_grid_selection"`에 기록한다.
...
- 실제 사건은 기사 제목, 역할별 아규먼트, 사건 흐름의 위치를 사실 그대로 말한다. 사용자 문장에는 `스레드`라는 내부 용어를 쓰지 않고 “처음 나온 소식”, “기존 발표의 후속 조치”, “협상 뒤 체결 단계”처럼 풀어 쓴다.
- “과거에 같은 패턴이 있어서 이렇게 됐다”는 문장을 쓰지 않는다. 영향 방향과 크기는 ATT가 통과한 경우에만 별도 문장으로 주장한다.
...
관세 정책 변경 사건이 있었어요. 제목은 “미국, 반도체 관세 변경 발표”이고, 발표자는 미국 정부, 대상은 반도체 수입품이에요. 이 정책에 관해 처음 나온 발표예요. 이 사건은 국내 시장을 위로 움직인 것으로 검정됐어요. {statistical, ev_1111111111111111}

반도체 수출 규제 변경 사건이 있었어요. 제목은 “첨단 반도체 수출 규제 확대”이고, 규제자는 미국 정부, 대상은 첨단 반도체예요. 앞서 발표한 규제의 범위를 넓힌 후속 조치예요. 이 사건은 반도체 업종을 위로 움직인 것으로 검정됐어요. {statistical, ev_2222222222222222}

삼성전자의 실적 발표 사건이 있었어요. 제목은 “삼성전자 분기 실적 발표”이고, 발표자는 삼성전자예요. 이번 분기 실적을 처음 공개한 소식이에요. 이 사건은 ETF의 고유한 부분을 위로 움직인 것으로 검정됐어요. {statistical, ev_3333333333333333}
```
