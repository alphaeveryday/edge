...
- **배치 = Step Functions** — 시장 레인 data-pipeline(raw→normalize→feature→analyze 4페이즈, 구 analysis-engine SFN 흡수 — ALPHA-408)을 `ecs:runTask.sync` 로 오케스트레이션(재시도·실패알림). 4페이즈는 **시장 SFN 만의 형태**다 — 뉴스·장중 수급은 analyze 페이즈가 없는 별도 state machine 이고, 공시 state machine은 ALPHA-1071의 평일 19:30 보충 배치를 담당한다. 구 임시 news-pipeline SFN 은 ALPHA-549 에서 제거 — `pipeline` 모듈은 lake 버킷만 존치.
