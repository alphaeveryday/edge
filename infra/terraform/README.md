...
>
> **Reconciler(`edge-dev-data-pipeline-reconcile`) 도 ENABLED** — `rate(15 minutes)`(컷오버, ALPHA-588). 이게 꺼져 있으면 `orchestration_status` 가 영영 NULL 이고, **자기 원장 기록이 불가능한 작업**(deepseek 처럼 task-def 에 DB env 가 없는 것 — ALPHA-181 의 명시적 제외)의 attempt 가 영영 비어 실제로 성공한 작업이 PENDING 으로 남는다. ⚠️ 주기 실행은 **스케줄 슬롯 하나만** 대조한다 — 수동 슬롯(`plan-run` 으로 만든 것)은 `OPS_RUN_KEY` 를 명시해 돌려야 대조된다(ALPHA-565 가 해소 예정).
>
