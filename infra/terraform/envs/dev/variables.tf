...
# ⚠️ RDS 쪽 둘도 같은 토픽·같은 구독에 달렸다 — 메모리 고갈 경보(ALPHA-919, 죽기 **전** 예고)와
# RDS 이벤트 구독(ALPHA-928, 죽었다는 **통보**). 여기를 null 로 두면 파이프라인 실패 통보만이
# 아니라 그 둘이 함께 사라진다 — 그게 2026-08-10 사고(하루 다섯 번 다운, 아침 09:30 경고
# 신호를 아무도 못 봄)의 실패 모드 그대로다. 알람 하나로는 안 된다: 그날 22:14 사망은 알람이
# 끝내 발화하지 않았고, 죽음을 통보하는 축은 이벤트 구독뿐이다.
variable "pipeline_alarm_email" {
  description = "파이프라인 실패·RDS 경보·RDS 이벤트 알림 수신 이메일. null 이면 SNS 구독 없이 토픽만(=알림 유실)."
  type        = string
