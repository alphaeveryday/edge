...
# 실행 역할: task 기동 시 주입하는 시크릿(DeepSeek 키·DB 비밀번호)만 읽는다.
resource "aws_iam_role_policy" "execution_secrets" {
  ...
  policy = jsonencode({
    ...
    Statement = [{
      ...
      Resource = [var.deepseek_secret_arn, var.db_password_secret_arn]
    }]
  })
}

# 태스크 역할: 런타임 최소권한 — canonical lake 읽기 + 설명 결과 prefix 쓰기.
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        Effect   = "Allow"
        Action   = ["s3:PutObject"]
        Resource = ["${var.lake_bucket_arn}/${var.result_s3_prefix}*"]
      },
    ]
  })
}
...
# runTask.sync 최소권한: ECS RunTask/StopTask/DescribeTasks + PassRole(task/exec) +
# 관리형 이벤트 룰(StepFunctionsGetEventsForECSTaskRule) + 실패 알림 SNS publish.
# StopTask/DescribeTasks 는 리소스 지정 불가(*)라 ecs:cluster 조건으로 워커 클러스터에 한정한다.
