...
#
# 🔴 **desired 를 0 으로 내리는 주체는 이제 여기뿐이다**(ALPHA-912 PR B — 세션이 손을 뗐다).
# 세션의 20:05 하드스톱은 CloudWatch 를 안 보는 유일한 천장이었다(대신 처리 중인 설명을
# 잘랐다). 이 알람·정책이 지워지거나 지표 축이 어긋나면 아무도 안 내려 밤새 공유 RDS
# 커넥션을 문 채 돈다 — 위에서 "가드레일"이라 부른 것이 **단일점**이라는 뜻이다.
# ⚠️ 밤을 넘기는 잔여가 실재한다: 08-10 은 20:05 시점 잔여 10(계단 2대)이었고 다음날
# 01:25 까지 안 비었다. 일을 마저 한다는 뜻이라 의도에 반하지는 않으나 상한 시간이 없다.
# 관측 항목이다(밤 잔여가 실제로 배출되는가 · 그때 DB 여유가 남는가).
...
resource "aws_cloudwatch_metric_alarm" "analysis_backlog" {
  ...
  # ⚠️ **결측은 실제로 난다.** SQS 는 큐가 **6시간 무접근이면 잠들어** 지표를 아예 안 보낸다
  # (CloudWatch 계약). 08-10 실측 공백은 **03:00~07:52 KST(4시간 52분)** 이고, 재개 시각은
  # 소비자 기동(07:47)의 5분 뒤다 — 메시지가 아니라 **폴링이 큐를 깨웠다**.
  # 그래서 이 값이 야간 동작을 정한다 — 유휴가 곧 결측이니 "잔여 없음"으로 읽는
  # `notBreaching` 이 맞다. `breaching`·`missing` 으로 바꾸면 밤새 대수가 남거나 직전
  # 상태가 고착된다.
  # ⚠️ 깨어날 때 지표에 **최대 15분** 지연이 붙는다(같은 계약) — 그날 첫 트리거의 스케일업이
  # 그만큼 늦는다. ALPHA-912 PR B 로 아침 폴링이 사라지면서 이 경로가 상시화됐다.
