...
# 🔴 그리고 ALPHA-912 PR B 뒤로 **여기 말고 desired 를 0 으로 내리는 주체가 없다** — 세션의
# 20:05 하드스톱이 사라졌다. 그건 CloudWatch 를 안 보는 유일한 천장이었다(대신 처리 중인
# 설명을 잘랐다). 이 알람·정책이 지워지거나 지표 축이 어긋나면 아무도 안 내려 밤새 공유
# RDS 커넥션을 문 채 돈다. "가드레일"이 이제 **단일점**이라는 뜻이다.
