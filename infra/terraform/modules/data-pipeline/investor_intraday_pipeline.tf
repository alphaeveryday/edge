...
locals {
  ...
  #
  # raw 부분 실패는 `status=partial`·`failed_records` 로 계속 드러낸다. 고립 심볼 1건은
  # `--max-failed-symbols 1`(이 레인이 재사용하는 statemachine.tf 정의)로 exit 0 을 허용해
  # 오케스트레이터 전체 FAILED 만 막는다. 2건 이상·0건 저장·스캔 미완료는 계속 비영이다.
  # 이 선은 유니버스 325종목을 종목당 1콜로 도는 실측에서 정했다.
  #
  # 2026-08-06 dev 실측에서 650콜 중 1건이 일시적 네트워크 오류(`Remote end closed
  # connection`)로 빠졌다 — 콜당 0.15% 면 슬롯당(325콜) 약 39%, 하루 5슬롯이면 **~2회**다.
  # 표본이 2런이라 참값 폭은 넓지만 자릿수는 이렇다. 재시도 사이에 못 받은 하나는 다음 슬롯의
  # 누적 응답이 회수하고, 손실은 원장의 `failed_records` 로 남아 대시보드에 INCOMPLETE 로 보인다.
  # 마지막 s1435도 같은 선을 쓴다. 뒤 자동 슬롯은 없지만 당일에는 수동 plan-run으로 누적 응답을
  # 회수할 수 있고, 그 전까지 INCOMPLETE가 복구 필요성을 표시한다. 여기만 exit 1로 되돌리면
  # ALPHA-798이 없애려는 고립 1심볼 오경보가 마지막 슬롯마다 그대로 남는다.
  #
