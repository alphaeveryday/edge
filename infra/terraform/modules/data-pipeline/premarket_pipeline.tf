...
#   ③ **실행이 아예 안 생김**(StartExecution 거부·역할 오류·스로틀) → 스케줄러 DLQ 도착 알람
#      (`ops_ledger.tf` `scheduler_dlq_arrivals`). ①②는 실행이 있어야 울므로 이 경로를 못 잡는다
#      — 그 알람을 이 레인 때문에 신설했다.
#
# 🔴 **그래도 남는 구멍 하나**: 스케줄이 드리프트로 꺼지거나 지워지면 전달 시도 자체가 없어
# ①②③ 어느 것도 안 운다(그날 유니버스는 어제 것 그대로다). 평일 cron 이라 주말·공휴일엔
# 실행이 없는 것이 정상이므로 "오늘 실행이 있었나" 류의 하트비트는 **달력을 알아야** 하고,
# 이 레포에서 달력을 아는 판정자는 원장 Reconciler 뿐이다 — 즉 이 구멍의 정답은 아래
# ⏭ 원장 편입이다. 그 전까지는 terraform 이 매 apply 마다 스케줄을 되살린다는 것이
# 유일한 방어다(드리프트가 오래 살아남지 못한다).
#
