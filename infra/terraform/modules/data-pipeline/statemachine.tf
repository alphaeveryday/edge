...
locals {
  ...
  kr_ingest_jobs = [
    ...
    # 과거 실패일은 수동 ECS 실행에서 같은 날짜의 --from/--to를 주어 단일 거래일 백필한다
    # (ALPHA-1063). 정규 SFN은 인자를 생략해 오늘/직전 거래일 규약을 그대로 쓴다.
      ...
      LoadInstruments = merge(local.ecs_run_task_base, {
        ...
        Parameters = merge(local.ecs_run_task_base.Parameters, {
          ...
          Overrides = {
            ContainerOverrides = [{
              Name        = local.container_name
              "Command.$" = "States.Array('load-instruments', '--run-id', $.run_id, '--latest-good')"
