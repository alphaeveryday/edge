...
#
# 패턴이 "수집 절단" 인 이유 — 이 토큰은 `sources/bigkinds.py` 가 **판정 근거(응답의
# totalCount)를 실제로 보고** 낼 때만 쓴다. 두 경우가 탄다:
#   ① `수집 절단 — N건 중 M건 수집, N-M건 유실` — 유실 건수가 경고 안에 있다(주 경로)
#   ② `수집 절단 판정 불가 — totalCount 없음` — 근거 필드가 사라진 경우. 건수는 모르지만
#      **완주를 주장할 수 없게 된 사건**이라 조용히 넘기지 않는다(Rule 12)
# 다른 수집기의 "MAX_PAGES 도달 — 창 절단 **가능**" 은 일부러 뺐다: 그건 근거를 안 보고
# 페이지 소진만으로 하는 추측이라 확인할 수 없는 경보가 된다. 그 소스들에 같은 근거를 주는
# 건 별건이다(그때 이 패턴에 합류시켜라).
#
