...
<div class="ep-version-note">
  ...
  <p data-version="v1" hidden>v1(v0.1)은 단일 클라우드 SaaS 로 그린 애플리케이션 구성이다. 콘솔 UI 6종이 도메인별 API 로 배선되고 위젯 UI 는 별도 사용자 표면으로 두며, 데이터 수집·처리와 종목 분석 엔진·AI 모듈이 같은 환경 안에서 위젯 화면까지 이어진다. 이 Cloud-only 구조는 폐기된 방향이다.</p>
</div>
