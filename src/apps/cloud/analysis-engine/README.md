...
- **산문의 요구창 봉도 주입받는다**(ALPHA-892) — `window_facts` 는 요구창·헤더창 봉을 호출자에게서 받고(`asked_bars`·`state_bars`), 파이프라인은 층 분해와 **같은 집계분**(`slice_requested_bars`·`aggregate_window`)을 넘긴다. 레이크 5분봉을 재질의하면 ① 라우팅이 판정에 쓴 가격과 갈리고 ② 그 `ts` 가 5분 격자점뿐이라 좁은 요구창(예: 09:06\~09:08)이 봉을 하나도 못 골라 `IntervalError` 로 죽는다 — 지연이 아니라 **기다려도 안 사는** 실패다(2026-08-11 실측 12건 중 10건). 봉을 모르는 호출자(`window_batch`·`explain` 단독)만 레이크를 읽고, 갭(`_gap`)의 전 거래일 종가·괴리(`premium_5m`)는 여전히 레이크 축이라 **헤더는 출처가 둘**이다(`lineage.read_via` 가 실제 읽은 경로를 말한다 — `view` 는 근거 게이트 키라 `bars_5m` 으로 고정).
- **요청창 런은 사건 분포 준비도를 후보 단위로 남긴다**(ALPHA-929) — 창 안 사건 후보마다 링크 판정(사유 코드)·preview 상태(`READY`/`UNAVAILABLE`+사유)·제출·렌더 결말이 `stage_results.window.event_distribution_observations` 와 `event_distribution.outcome` 로그에 남고, 리포트 발화 상세에 표로 렌더된다. 예외로 끊긴 런도 그때까지의 관측을 flush 하며 payload 의 `collection`(`COMPLETE`/`ABORTED`)이 "확인된 0건"과 "조회 미도달"을 가른다. 사건 수익률 표면이 죽은 런(`event_return_surface.status != READY`)은 `degraded` 로 영속돼 확신도가 올라가지 않는다(Rule 12).
