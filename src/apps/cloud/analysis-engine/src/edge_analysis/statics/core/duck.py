...
class CausalLake:
    ...
    def taus_many(self, instrument_ids: tuple[str, ...], day: str) -> list[tuple]:
        """여러 종목의 그날 사건을 한 번에 읽는다: (instrument_id, τ, event_id)."""
        if self.exists.get("rdb") is not True:
            raise RuntimeError("RDB 부재 — coverage 참조")
        ids = tuple(dict.fromkeys(str(value) for value in instrument_ids))
        if not ids:
            return []
        promote = ""
