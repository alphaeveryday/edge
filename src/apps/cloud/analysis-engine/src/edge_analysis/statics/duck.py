...
#
# ⚠️ data-pipeline 의 `minute.rollup.WRITER_SINCE` 와 **같은 값이 아니다**(ALPHA-836 에서
# 갈렸다). 저건 "롤업이 파티션을 쓰는 소유권 경계"라 운영 판단으로 움직이고, 이건
# "롤업이 실제로 공급하기 시작한 날"이라 **역사적 사실**이다 - 안 움직인다. 둘이 같아
# 보인다고 맞추면 경계가 옮겨질 때마다 이 심판축까지 따라가 과거 구간의 벤더 중재가
# 조용히 바뀐다.
...
def iceberg_covers(newest, asked_day: str, day_tickers: int = 0,
    ...
    **최신 거래일도, 그날의 행 수도 아니라 착지 종목 수를 본다.** 앞의 두 자는 각각
    한 단계씩 새는 자다:

    - `max(trade_date) >= 요청일` — 표에 구멍이 있어도 통과한다
    - `그날 행 > 0` — **부분 착지가 그대로 통과한다.** 실측 8/3·8/4 는 13종이었는데
      그 안에 `069500`(시장)이 들어 있었다. 즉 "시장 지수가 있나" 로도 못 거른다.
      13종이 통과하면 나머지는 `_on()` 이 조용히 버리고, 남은 층은 그 13종으로 세운
      것인데 산문은 그 사실을 말하지 않는다 — 이 저장소가 계속 싸워 온 조용한 부재다.

    임계는 지어낸 값이 아니라 **관측 분포의 빈 구간**이다 (Athena 실측 2026-08-07):

    | 구간 | 착지 종목 |
    |---|---|
    | 07-20~07-31 (fmp 시대 정상일) | 1,267~1,273 |
    | 08-05 (1분 롤업 시대 정상일) | 366 |
    | **08-03·08-04 (부분 착지)** | **13** |

    두 무리 사이가 28배로 벌어져 있어 그 사이 어디를 잘라도 같은 답이 나온다.
    100 은 부분 착지의 7.7배 · 롤업 정상일의 27% 로 양쪽에 한 자릿수 배율 마진이 있다.
    **정상일 폭이 시대마다 다르므로**(1,270 vs 366) 티켓이 적은 '1,200종' 같은 절대
    기준은 못 쓴다 — 롤업 시대 전일이 폴백이 된다.

    장중에도 같은 자가 선다: 08-05 실측에서 **09:00 첫 버킷부터 362종**이었다(롤업이
    ...
    def _bars_iceberg(self) -> bool:
        ...
        **조용히 과거를 본다** - 최신 거래일이 문자열에 있으면 사람이 그걸 본다.
        ...
        try:
            ...
            # 최신일과 **그날의 착지 종목 수**를 함께 묻는다 - 최신만 보면 표에 구멍이
            # 있어도 통과하고, 행 수만 보면 **부분 착지가 그대로 통과한다**.
            # `asked_day` 가 없으면 `= NULL` 이 되어 종목 수는 0 이고, 그때는
    ...
    def _bars(self, d: Path) -> None:
        ...
        **중복 제거는 생성 시점의 서로소 구성이다 — 질의 시점 창은 없다.** 처음엔
        ...
        그래서 겹칠 수 있는 곳만 **작은 표로 미리 계산해 안티조인**한다:
          · S3 내부 벤더 겹침은 **0 이도록 상류가 보장한다** - 한 파티션에 여러 writer 의
            파일이 있어도(정본 `part-0` + 벤더 백필) 행은 **티커 단위로 서로소**다:
            백필이 쓰기 전에 정본의 티커를 빼고, 롤업은 자기 소유 구간에서 낯선 파일을
            만나면 산출을 멈춘다. 없는 겹침에 값을 치르지 않는다.
            ⚠️ 파티션이 비어 있다는 관측을 근거로 삼지 않는다 - 백필이 채우면 그 관측은
            바로 뒤집힌다(ALPHA-836 이 08-03·08-04 를 그렇게 채웠다). 근거는 상류의
            서로소 보장이지 그날의 행 수가 아니다.
