...
def sector_proxy(lake, tk6: str, day: str, topn: int = SECTOR_TOPN) -> dict:
    ...
    ALPHA-887 이 45종 1분봉을 수집한다(dataset `sector_index_minute`). 못 쓰는 이유는
    둘: ① 그 dataset 을 읽는 배선이 이 경로에 없고, ② 소급 TR 이 일봉으로 degrade 해
    레인이 도는 날 이전은 영구 결손이다(②는 배선을 놓아도 남는다). 두 함정:
