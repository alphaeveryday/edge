...
⚠️ **"업종지수는 분봉이 없다"는 거짓이다**(ALPHA-887 이 45종 1분봉을 dataset
`sector_index_minute` 으로 수집한다). 정확히는 **이 경로가 아직 못 읽는다** - ① 여기가
읽는 것은 `bars_5m` 인데 그 dataset 은 1분 정본뿐이고 5분 롤업 배선이 없다, ② 소급 TR 이
일봉으로 degrade 해 레인이 도는 날 이전은 영원히 없다. 그래서 구간(clock) 모드의 섹터
층은 사유를 남기고 시장+고유 2층으로 선다. 프록시로 메우면 '가장 잘 맞는 무엇'이 섹터를
참칭하던 그 자리로 돌아간다.
"""
...
def decompose(lake, etf: str, day: str, *, clock: tuple[str, str],
    ...
    층은 **정직 부재**다(ALPHA-877) - KRX 업종지수 1분봉은 ALPHA-887 이 수집하지만
    이 경로가 못 읽는다(5분 롤업 없음 · 소급 불가 — 모듈 도크스트링), 프록시 섹터
    ETF 로 메우던 겹침 선택은 폐기했다. 사유를 커버리지에 남기고 시장+고유 2층으로 선다.
