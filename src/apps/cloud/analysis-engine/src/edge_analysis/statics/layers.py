...
놓아도 남는다). 그래서 섹터 층은 당분간 수집된 섹터 ETF 중 겹침으로 고른 하나가 선다
(`select_sector` — ALPHA-877 이 걷었던 프록시 선택을 #659 가 칼만 배선과 함께 되살렸다).
후보가 없으면 사유를 커버리지에 남기고 시장+고유 2층으로 선다.
"""
...
def decompose(lake, etf: str, day: str, *, clock: tuple[str, str],
    ...
    섹터 ETF 중 겹침으로 고른 하나가 선다(`select_sector`) - 후보가 없으면 사유를
    커버리지에 남기고 시장+고유 2층으로 선다. 정본인 KRX 업종지수는 1분봉을 수집해도
    (ALPHA-887) 이 경로가 아직 못 읽는다(5분 롤업 없음 · 소급 불가 — 모듈 도크스트링).
