...
# 창이 이만큼 안 차면 그 층은 후보에서 빠진다 (판정불가지 0 이 아니다).
#
# 40 → 20 (ALPHA-849). **커버리지 판단이지 통계적 판단이 아니다.** 40 을 물리적으로 못
# 채우는 계열이 있고, 그 계열을 쓰는 ETF 는 섹터 층이 통째로 없다 - 실측(2026-08-08,
# ALPHA-836 백필 직후): 섹터 계열 80종 중 40일 충족 79종, 남은 0210A0 은 상장이 늦어
# 33일이 최대다. 20 이면 80/80 이다.
#
# ⚠️ **대가가 있다.** 표본이 줄면 `_ols` 의 se 가 커지고(dof=n−2) `_z` 는 정규분위수라
# t 와의 괴리가 벌어진다 - 후보 80종 Bonferroni 에서 실제 familywise α 가 n=40 의 0.121
# 에서 n=20 의 0.244 로 는다(실측). 즉 **위양성이 늘어난다**. 그걸 알고 내린 것이고,
# 게이트 자체(정규→t)는 이 티켓의 범위 밖이다. 층이 하나 더 서는 것과 없는 것 중
# 지금은 서는 쪽을 택했다 - 뒤집으려면 게이트를 먼저 고쳐라.
MIN_BETA_N = 20
