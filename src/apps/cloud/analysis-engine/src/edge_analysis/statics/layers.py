...
BETA_WINDOW = 60      # 계열 창 (거래일). 당일 제외 - 오늘 충격이 β 를 오염시키면 안 된다
# 창이 이만큼 안 차면 그 층은 후보에서 빠진다 (판정불가지 0 이 아니다).
#
# 40 → 20 (ALPHA-849). 40 은 **β 를 회귀로 추정하던 시절**의 요건이다 - 계수 신뢰구간이
# 서려면 그만큼 필요했다. 계수를 1 로 고정한 뒤로 이 창이 실제로 지탱하는 것은 셋이고
# (직교화 · 층 선택 · ρ 게이트) 전부 회귀 추정보다 얇은 표본으로 선다: 직교화는 공분산
# 하나, 층 선택은 잔차 크기 비교, ρ 게이트는 상관 부호다.
#
# 실측(2026-08-08, ALPHA-836 백필 직후): 섹터 계열 80종 중 40일 충족 79종. 남은 0210A0
# 은 33일로 **상장이 늦어 물리적으로 40 을 못 채운다** - 20 이면 80/80 이다.
# 20 아래로는 안 내린다. 그 밑에선 직교화가 표본 잡음을 층 기여로 옮긴다.
MIN_BETA_N = 20
