...
45종 1분봉을 dataset `sector_index_minute` 으로 수집한다. 정확히는 **이 경로가 아직 못
읽는다** - ① 여기가 읽는 것은 `bars_5m` 인데 그 dataset 은 1분 정본뿐이고 5분 롤업 배선이
없다, ② 소급 TR 이 일봉으로 degrade 해 레인이 도는 날 이전은 영원히 없다(②는 배선을
놓아도 남는다).

그동안의 섹터 층은 **수집된 섹터 ETF 중 겹침으로 고른 하나**다(`select_sector` —
ALPHA-877 이 걷었던 프록시 선택을 #659 가 칼만 배선과 함께 되살렸다). 겹침은 설명력이
아니라 구성 중복이라 위 금지에 걸리지 않지만, 최종형이 아닌 것은 그대로다. 선택은
**층이 서는 필요조건일 뿐이다** - 2요인 칼만이 `sector_contribution` 을 못 내면(정렬된
경로 부재) 층은 안 서고 사유가 커버리지에 남는다. 후보 자체가 없을 때도 같다.
"""
...
def decompose(lake, etf: str, day: str, *, clock: tuple[str, str],
    ...
    뿐이고, 2요인 칼만이 `sector_contribution` 을 못 내면 층은 안 선다. 후보가 없거나
    정렬된 경로가 없으면 사유를 커버리지에 남기고 시장+고유 2층으로 선다. 정본인 KRX
    업종지수는 1분봉을 수집해도(ALPHA-887) 이 경로가 아직 못 읽는다(5분 롤업 없음 ·
    소급 불가 — 모듈 도크스트링).
