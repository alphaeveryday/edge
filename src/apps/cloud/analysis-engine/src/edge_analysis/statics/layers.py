...
45종 1분봉을 dataset `sector_index_minute` 으로 수집한다. 레이크 경로가 못 읽는 이유는
둘이다 - ① 레이크에서 읽는 것은 `bars_5m` 인데 그 dataset 은 1분 정본뿐이고 5분 롤업
배선이 없다, ② 소급 TR 이 일봉으로 degrade 해 레인이 도는 날 이전은 영원히 없다(②는
배선을 놓아도 남는다).

🔴 **그러나 롤업만 놓아도 주 실행 경로는 안 바뀐다.** 운영 런(`pipeline`)은 레이크가
아니라 **커밋된 1분봉**을 주입하는데, 그 집합이 대상·시장·구성종목뿐이라(`pipeline` 의
`needed`) `select_sector` 에 넘어가는 `available` 에 섹터 계열이 아예 없다 → 후보가 비고
섹터 층은 부재다. 업종지수를 실제로 쓰려면 **그 주입 집합에 넣는 일**이 따로 있어야 한다
(파이프라인 주석: "참조 계열은 분석 unit 밖 - 후속 PR").
...
def decompose(lake, etf: str, day: str, *, clock: tuple[str, str],
    ...
    사유를 커버리지에 남기고 시장+고유 2층으로 선다. ⚠️ **운영 런은 후보가 비어 있다**
    - 호출자가 주입하는 `intraday` 가 대상·시장·구성종목뿐이라 `available` 에 섹터
    계열이 없다(모듈 도크스트링). 정본인 KRX 업종지수도 1분봉을 수집해도(ALPHA-887)
    아직 그 주입 집합 밖이다.
