...
구간(clock) 모드의 섹터 층은 **정직 부재**다 - 사유를 남기고 시장+고유 2층으로 선다.
"업종지수는 분봉이 없다"가 이유였지만 그건 이제 거짓이다(ALPHA-887 이 1분 레인의
dataset `sector_index_minute` 으로 45종을 수집한다). 정확히는 **이 경로가 아직 못
읽는다** - 근거 셋: ① 구간 모드가 읽는 것은 `bars_5m` 인데 그 dataset 은 1분 정본뿐이고
5분 롤업 배선이 없다, ② 소급 TR 이 일봉으로 degrade 해 레인이 도는 날 이전은 영원히
없다, ③ 소비 배선이 0 이고 장중 실런도 아직 0 회다(코드 머지 ≠ 실효). 셋 중 하나라도
남아 있는 한 프록시로 메우면 안 된다 - '가장 잘 맞는 무엇'이 섹터를 참칭하던 그
자리로 돌아간다.
"""
...
def decompose(lake, etf: str, day: str, *, top: int = TOP_NAMES,
    ...
    층은 **정직 부재**다(ALPHA-877) - KRX 업종지수 1분봉은 수집하지만(ALPHA-887) 이
    경로가 못 읽고(5분 롤업 없음·소급 불가 — 모듈 도크스트링), 프록시 섹터 ETF 로
    메우던 겹침 선택은 폐기했다. 사유를 커버리지에 남기고 시장+고유 2층으로 선다.
    ...
    if layers and not broad:
        if clock is not None:
            # 업종지수 1분봉은 수집한다(ALPHA-887) - 이 경로가 못 읽을 뿐이다:
            # 5분 롤업이 없고 소급도 안 된다(모듈 도크스트링). 프록시 ETF 로 메우면
            # '가장 잘 맞는 무엇'이 섹터를 참칭하던 자리로 돌아간다. 정직 부재.
            _absent(lake, "sector_layer",
                    "업종지수 5분봉 없음(1분봉만 수집·소급 불가) - 시장+고유 2층")
        else:
