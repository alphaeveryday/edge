...
def design(ask, lake, *, etype: str, day: str, layer: str, brief: str = "",
    ...
    primary = ask_checked(ask, system, user)
    probes, bad = screen_probes(
        list_field(primary, "probes"), menu, max_probes=max_probes)
    if len([p for p in probes if p["slots"]]) == 0 and any(
        ...
        retry = (user + "\n\n직전 제출이 하나도 쪼개지 않았다. 메뉴에 값이 있으니 "
                 "최소 두 개는 슬롯을 채워 내라.\n" + "\n".join(bad[-4:]))
        retried = ask_checked(ask, system, retry)
        more, bad2 = screen_probes(
            list_field(retried, "probes"), menu, max_probes=max_probes)
        bad += bad2
