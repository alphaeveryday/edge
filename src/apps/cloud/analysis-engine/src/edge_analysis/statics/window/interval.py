...
3. **코드가 구조와 수치를 소유한다.** `BlockPlan`이 블록 순서·발생·근거를 정하고,
   LLM은 허용된 자리표시자만 채운다. 일단위 검정을 요청창 인과로 승격하지 않는다.
    ...
    python -m edge_analysis.statics.window.interval <ticker6> <instrument_id> <YYYY-MM-DD> \
        <HH:MM> <HH:MM>
"""
...
from ..core.layers import SESSION_CLOSE, SESSION_OPEN, decompose
...
def _final_lines(lake, ticker: str, day: str, event_ids: tuple[str, ...],
    ...
    rank = "CASE e.source_event_id " + " ".join(
        f"WHEN {_literal(eid)} THEN {i}" for i, eid in enumerate(event_ids)) + " END"
    try:
        events = lake.sql(
            "SELECT e.source_event_id, "
            "coalesce(any_value(doc.title), any_value(ev.evidence_text), "
            "         any_value(e.event_type_code)) "
            "FROM rdb.public.source_event e "
            "LEFT JOIN rdb.public.event_evidence ev "
            "  ON ev.source_event_id=e.source_event_id "
            "LEFT JOIN rdb.public.document_assertion da "
            "  ON da.assertion_id=ev.assertion_id "
            "LEFT JOIN rdb.public.document doc ON doc.document_id=da.document_id "
            f"WHERE e.source_event_id IN ({ids}) "
            "GROUP BY e.source_event_id "
            f"ORDER BY {rank} LIMIT 1")
...
def _requested_event_times(lake, etf_instrument_id: str, day: str,
                           names: tuple, start: dt.datetime,
                           end: dt.datetime) -> dict[str, dt.datetime]:
    """ETF와 기여 상위 구성종목에서 요청창 안에 실제로 알려진 사건만 모은다."""
    from .etfday import instrument_ids

    tickers = [str(item.ticker) for item in names if getattr(item, "ticker", None)]
    ids = tuple(dict.fromkeys(
        (etf_instrument_id, *instrument_ids(lake, tickers, day).values())))
    rows = (lake.taus_many(ids, day) if hasattr(lake, "taus_many") else
            [(iid, tau, event_id) for iid in ids
             for tau, event_id in lake.taus(iid, day)])
    found: dict[str, dt.datetime] = {}
    for _instrument_id, tau, event_id in rows:
        if tau is None or tau.time() == UNKNOWN_TAU or not start <= tau <= end:
            continue
        event_id = str(event_id)
        if event_id not in found or tau < found[event_id]:
            found[event_id] = tau
...
def window_facts(lake, ticker: str, instrument_id: str, day: str,
    ...
    event_times = _requested_event_times(
        lake, instrument_id, day, names, start, end)
