...
def test_committed_minute_reader_rejects_malformed_ndjson_and_ohlcv():
    ...
    # 전일 70,000 대비 -99.999% 가 '정상 수익률'로 인증된다(coerce-to-passing).
    # 파이썬에서 bool 은 int 의 하위형이라 `Decimal(str(value))` 만으론 안 걸린다.
    boolean = _strict_body(_strict_row("A", 0, close=True))
    reader = _reader({minute_artifact_key("KR", "2026-08-07", "0900", 1): boolean})
    with pytest.raises(PipelineError, match="close가 bool"):
        reader.load_committed_minute_bars("KR", [_committed(0, boolean)])
