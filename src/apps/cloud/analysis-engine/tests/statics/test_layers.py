...
(3) 섹터의 최종형은 **KRX 업종지수**다(ALPHA-877) - 다만 그 1분봉을 ALPHA-887 이
수집해도 5분 롤업·소급이 없어 이 경로가 아직 못 읽고("분봉이 없다"가 아니다),
그동안은 겹침으로 고른 섹터 ETF 가 후보로 선다(#659). 후보 선택은 필요조건일
뿐이라 정렬된 경로가 없으면 층은 안 선다 — 운영 런은 주입 집합에 섹터 계열이
없어 후보 자체가 비어 있다. 회귀·ρ·표본 게이트가
