...
> 판정 축으로 옮겼지만 **unit 수는 안 늘었다** — KODEX 은행 구성종목은 KODEX 200 경유로
> 이미 전부 `constituent_ids` 안이라 늘어난 건 etf_map 축 3작업(KRX PDF·NAV·프로필)의
> 각 1콜뿐이다. 콜 예산 재계산의 근거로 쓰지 마라. 토스 adapter 는 대체 소스로 남는다(`source=toss`). ⚠️ 뉴스 Consumer 는 실행 표면이 생겼고(ALPHA-713 —
...
# writer 파일 존재·백필 소유일 — 전부 재시도로 안 낫는다) / 2=판정 자체를 못 함
