...
- **raw(ETF iNAV)** — `raw/source=kis/dataset=etf_inav/market=KR/ingest_date=…/run_id=…/` 에
  ...
  **비율**이라 단위가 갈린다 — canonical 이 같은 이름을 쓰면 두 표면을 조인하는 쪽이 100배
  틀린 괴리를 본다. canonical 필드는 `premium_pct` 다(ALPHA-845).
  수집은 ETF 마다 두 가지를 관측한다: **벤더 지연**(수신시각 − 최신 `bsop_hour`)과 **단위
  가드**. 지연이 창 폭에 가까우면 이 API 로 장중 실시간이 성립하지 않는다 — 1분 레인 편입
  설계가 그 수치에 걸려 있다. 단위 가드는 어긋날 때만 경고한다(정상은 조용 — 확정된 사실을
  폴링마다 되풀이하면 진짜 신호가 묻힌다).
