...
- 사후 보강(PR 전 리뷰, 원시 결과 재생성 없음): `trial.py`가 ① 정상 요청 뒤·복구 뒤·재요청 뒤 대사 중 하나라도 업무 값 불일치이거나 원장 완료가 7/7이 아니면 비0 종료(A 복구 뒤 5/7은 현행 절차 관측값이라 제외), ② `load-price-daily` exit 2를 실패로 집계(계속 진행 규칙은 `normalize-price`만), ③ 호출 기록 조회 실패를 0건으로 접지 않도록, ④ Airflow task 로그 복사 실패를 실패로 끝내도록 고쳤다. r1·r2 원시 결과를 대조해 영향이 없음을 확인했다 — exit 2 호출 0건, 호출 기록 조회 실패 0건, 전 시나리오의 정상 요청 뒤(N)·복구 뒤·재요청 뒤(F) `business_ok=true`·원장 7/7(A 복구 뒤 5/7 제외). r1·r2는 수정 전 코드로 만들어졌다.
