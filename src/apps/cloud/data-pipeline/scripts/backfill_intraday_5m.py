...
(#617)가 층 계수를 전부 1 로 고정하며 회귀·ρ 게이트를 걷어냈다. ALPHA-877 은 프록시 섹터
ETF 후보 풀도 폐기했었지만 **#659 가 되살렸다** — `layers.select_sector` 가 수집된 섹터
ETF 를 겹침으로 골라 칼만에 배선한다. 그래도 이 파일의 근거는 안 돌아온다: 되살아난 것은
**겹침 선택이지 60일 β 회귀가 아니다** — `layers.BETA_WINDOW`·`layers.MIN_BETA_N` 은 여전히
없고, 시변 β 는 회귀가 아니라 칼만이 낸다. 섹터의 최종형인 KRX 업종지수는 1분봉을
수집해도(ALPHA-887, dataset `sector_index_minute`) 구간 모드가 읽는 5분 롤업 배선이 없어
아직 못 쓴다.

⚠️ **그러니 "β 표본 복구"로 이 파일을 정당화하지 마라.** `attribute.MIN_BETA_N` 이 아직
있지만 그건 **갭 β**(직전 미국 세션 대비)의 상수이고 대상 종목 **자신의** 전 이력을 센다.
참조 계열 47종은 `sector_etf_ids` 축이라 `price_consumer` 의 판정 대상이 아니므로
(`config/sources.toml` 의 그 목록 주석) 그 경로를 안 탄다 — **그 5분봉 이력의 살아
있는 소비자는 2026-08-09 현재 확인되지 않았다**(그때는 48종이었다. 091170 은
ALPHA-927 이 판정 축으로 옮겼고, 그래서 **그 한 종은 이제 이 문단의 예외다**).
