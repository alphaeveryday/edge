...
(`FHKST03010200`) 하나였고 그 TR 에는 날짜 파라미터가 없다. 토스는 `before` 로 과거를
거슬러 준다(실측: 305720 이 2025-09 까지 나온다).
⚠️ **"KIS 는 과거를 못 준다"로 읽지 마라** — 소급 TR `FHKST03010230` 이 따로 있고
ALPHA-846 이 그걸 `KisHistoricalMinuteClient` 로 붙였다. 그 경로는 1분 canonical +
원장 커밋을 내므로 계보가 남는다(이 스크립트는 5분봉을 직접 써서 계보가 없다) —
**새 결손일은 그쪽을 먼저 검토해라.** 5분봉 API 는 어느 벤더에도 없으므로 1분을 받아 롤업한다 —
...
🔴 **`rollup.WRITER_SINCE`(2026-08-03) 이후 파티션에는 쓰지 마라** (ALPHA-839 이후,
ALPHA-846 이 하한을 08-03 으로 낮췄다 — 08-03 은 이제 롤업 writer 의 날이다).
