...
쓴 것이고 2026-08-04 이후는 롤업 writer 가 5분마다 통째로 덮어쓴다. 거기에 끼워 넣으면
롤업이 다음 버킷에서 지운다. 같은 파티션에 `part-toss-backfill.parquet` 로 따로 쓴다 —
소비자(`duck._s3`)가 `**/*.parquet` 글롭이라 둘 다 읽고, 어느 쪽도 서로를 안 지운다.
# ponytail: 파티션당 1파일 관례를 깬다. 합치려면 롤업 writer 가 이 원천을 알아야 하고
# 그건 이 백필의 수명(일회성)보다 큰 변경이다.
