...
class MinutePriceWorkerConfig(BaseModel):
    ...
    # 이라 다른 kis 스텝과 나눠 쓴다 — 기본은 12 req/s 로 마진을 두고, 경합이 보이면
    # 재배포 없이 env 로 올린다. toss 소스에는 쓰이지 않는다(어댑터가 자기 상수).
    #
    ...
    # 즉 하루 5회, 각 ~3.5분씩 제시 유량이 12+2 = **14 req/s** 로 실측 상한 14.8 에 붙는다.
    # ponytail: 헤드룸 0.8 req/s. 1분 레인에 EGW00201 이 보이기 시작하면 여기(12)를 먼저
    # 내려라 — 장중 수급은 하루 5회뿐이라 그쪽 간격을 늘리는 것보다 값이 크다. 14.8 은
    # "달성 실측치"지 "거절 임계"가 아니라 레포 안에서는 여유의 참값을 알 수 없다.
