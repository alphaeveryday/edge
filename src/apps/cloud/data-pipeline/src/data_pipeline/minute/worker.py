...
class InavWorker(MinuteWorkerLoop):
    ...
    def _expected_units(self, window_start: datetime) -> tuple[str, ...]:
        """NAV 를 **부를 수 있는** unit 만 = 판정 축 `etf_ids`.

        기준은 **NAV 가 있는가**가 아니라 **부를 수 있는가**다. 질의 심볼의 출처는
        `krx_etf.source.etf_map` 하나뿐이고(진입점 `inav_worker_cli`) 그 맵 밖 unit 은
        `inav_collect._rows_for` 가 `Outcome.INVALID` 로 내므로, 맵 밖을 기대에 실으면
        `status_of` 가 **window 전체를 INVALID** 로 만든다(재청구 대상도 아니다). 시각
        게이트는 그대로 태우고(거래시간 밖 window 를 조용히 빈 집합으로 만들지 않는다 —
        `units_at` 이 거기서 raise 한다) 그 결과에서 판정 축 ETF 만 남긴다. 구성종목이
        빠지는 것도 같은 이유다 — NAV 도 없고 심볼 맵에도 없다.

        ⚠️ 판정 축이 **맵 안이라는 보장은 여기에 없다.** 빌더가 `etf_ids` 를 `etf_map` 에서
        파생하므로(`build_minute_universe` → `_kr_etf_ids(storage, _krx_expected_etfs(...))`)
        실무상 부분집합이지만, universe 객체와 config 는 따로 배포되는 두 입력이라 갈릴 수
        ...
        ⚠️ 이건 **NAV 축만의 판단이다.** 참조 계열의 **봉**은 가격 레인이 그대로 수집하고,
        그 봉을 누가 읽는지는 여기와 무관하다(그 축의 소비 상태는 ALPHA-904 소관 — config
        `[minute_universe]` 주석과 analysis `statics.layers` 가 서로 반대로 말하고 있다).
        """
        universe = self.config.universe
        nav_units = set(universe.etf_ids)
