...
#
# ⭐**값의 SSOT 가 여기인 이유**: KRX 를 읽는 경로가 둘이고 **어느 시장인지를 서로 다른 축**
# 으로 안다(ALPHA-829) — 비공식 getJsonData 는 응답의 `MKT_ID`(STK/KSQ/KNX)를 읽고, 공식
# OpenAPI 는 시장마다 엔드포인트가 갈려 **어댑터가 어느 엔드포인트를 불렀는지**로 안다.
# 각자 자기 표를 들면 한쪽만 고쳐도 아무도 못 잡는데, 그 결과는 **같은 종목이 두
# market_code 로 갈려 마스터에 두 번 서는 것**이다(자연키가 `(market_code, ticker)`).
# 시장 판정 방식은 각 정제단이 갖되, **보드→MIC 값은 여기 하나**를 쓴다.
KR_MIC_BY_BOARD = {
    "KOSPI": "XKRX",   # 유가증권시장
    "KOSDAQ": "XKOS",
    "KONEX": "XKON",
...
def krx_short_code(value: object) -> str | None:
    ...
    **숫자 6자리가 아니다** — KRX 가 번호를 소진해 신규 상장분에는 문자가 섞인 코드
    (0093A0·0005G0 등)를 발급하고, 우리 ETF 유니버스 38종 중 8종이 그렇다. `isdigit()`
    로 보면 그 8종이 조용히 빠진다(ALPHA-463·380 이 각각 가격·NAV 에서 겪은 결함).
