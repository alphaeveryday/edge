...
#
# 60 → 70(ALPHA-569): 이 엔드포인트는 자기 쿼리가 안 끝나면 **서버가 60~61초에 400 QUERYTIMEOUT
# 으로 스스로 끊는다**(2026-07-27 열화 장애 라이브 실측 8회 60.02~61.12초 — 클라이언트 타임아웃을
# 300초로 열어도 동일). 우리 타임아웃이 그 컷보다 작으면 400 이 도착하기 전에 TimeoutError 가
# 나고, 운반 계층이 네트워크 오류로 분류해 재시도 4회를 태운다(ETF당 최대 188초 스톨 — 07-27
# 장애의 100분 스톨 원인). 컷(≈61초)을 넘겨 둬야 서버 판정이 먼저 도착해 QUERYTIMEOUT ETF 단위
# 격리(krx_etf._fetch_etf)가 동작한다.
KRX_ETF_TIMEOUT_SEC = 70.0
