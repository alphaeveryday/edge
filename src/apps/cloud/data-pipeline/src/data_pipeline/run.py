...
def make_run_id(now: datetime | None = None) -> str:
    """run_id(YYYYMMDDTHHMMSSZ) — 인자 없으면 현재 UTC. `now` 는 변환 없이 그대로
    포맷하므로 UTC aware 를 넘겨야 접미사 Z 가 사실이 된다."""
...
def _dispatch(args, settings, storage, run_id) -> int:
    ...
    # 투자자 수급 적재 정상 경로는 NormalizeInvestor manifest의 직접 key와 winner만 읽는다.
    # 날짜창은 명시 복구, 전체 스캔은 --all을 직접 쓴 경우뿐이다(ALPHA-1041).
    if args.step == "load-etf-flow":
        ...
        if scopes != 1:
            raise SystemExit(
                "load-etf-flow는 --input-run-id, --from/--to, --all 중 하나가 필요하다"
            )
        if (args.from_date is None) != (args.to_date is None):
            raise SystemExit("load-etf-flow의 --from과 --to는 함께 써야 한다")
        parsed_dates = []
        for name, value in (("--from", args.from_date), ("--to", args.to_date)):
            if value is None:
                parsed_dates.append(None)
            ...
            try:
                ...
                raise SystemExit(
                    f"load-etf-flow의 {name}은 YYYY-MM-DD 달력일이어야 한다"
                ) from exc
            if parsed.strftime("%Y-%m-%d") != value:
                raise SystemExit(
                    f"load-etf-flow의 {name}은 YYYY-MM-DD 달력일이어야 한다"
            ...
            parsed_dates.append(parsed)
        if all(parsed_dates) and parsed_dates[0] > parsed_dates[1]:
            raise SystemExit("load-etf-flow의 --from은 --to보다 늦을 수 없다")
    ...
    # 장중 투자자 추정 적재의 정상 경로는 NormalizeInvestorEstimate manifest의 직접 key와
    # winner만 읽는다. 날짜창은 명시 복구, 전체 스캔은 --all을 직접 쓴 경우뿐이다(ALPHA-1036).
    if args.step == "load-investor-intraday":
        ...
        if scopes != 1:
            raise SystemExit(
                "load-investor-intraday는 --input-run-id, --from/--to, --all 중 하나가 필요하다"
            )
        parsed_dates = []
        for name, value in (("--from", args.from_date), ("--to", args.to_date)):
            if value is None:
            ...
            try:
                ...
                raise SystemExit(
                    f"load-investor-intraday의 {name}은 YYYY-MM-DD 달력일이어야 한다"
                ) from exc
            if parsed.strftime("%Y-%m-%d") != value:
                raise SystemExit(
                    f"load-investor-intraday의 {name}은 YYYY-MM-DD 달력일이어야 한다"
            ...
            parsed_dates.append(parsed)
        if all(parsed_dates) and parsed_dates[0] > parsed_dates[1]:
            raise SystemExit("load-investor-intraday의 --from은 --to보다 늦을 수 없다")
    ...
    # 정상 가격변동 트리거는 NormalizePrice manifest가 지정한 현재 winner를 DB 가격·holdings와
    # 결합한다. 날짜창은 명시 복구, 전체 스캔은 --all을 직접 쓴 경우뿐이다(ALPHA-1039).
    if args.step == "load-price-triggers":
        ...
        if scopes != 1:
            raise SystemExit(
                "load-price-triggers는 --input-run-id, --from/--to, --all 중 하나가 필요하다"
            )
        if (args.from_date is None) != (args.to_date is None):
            raise SystemExit("load-price-triggers의 --from과 --to는 함께 써야 한다")
        parsed_dates = []
        for name, value in (("--from", args.from_date), ("--to", args.to_date)):
            if value is None:
                parsed_dates.append(None)
            ...
            parsed_dates.append(parsed)
        if all(parsed_dates) and parsed_dates[0] > parsed_dates[1]:
            raise SystemExit("load-price-triggers의 --from은 --to보다 늦을 수 없다")
        return load_price_triggers.run(
            storage, run_id, db=db_config_from_env(settings.db),
            config=settings.price_triggers, input_run_id=args.input_run_id,
            from_date=args.from_date, to_date=args.to_date,
            expected_etfs=ingest_price_raw._krx_expected_etfs(settings),
        )
