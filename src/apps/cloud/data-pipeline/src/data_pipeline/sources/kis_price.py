...
def _stop_bound(d1: str) -> str:
    ...
    ⚠️ **남는 구멍**: 벤더가 **비종단 short page**(꽉 안 찼는데 아래에 더 있음)를 주고 *동시에*
    휴장일 집합이 과잉이면 그 날 봉을 놓친다. 과잉 등재 하나만으로는 안 생긴다 — 요청 하한이
    원본 d1 이라 그 날이 실제로 거래됐으면 응답에 들어오고, 꽉 찬 페이지에선 달력을 안 믿는다.
    비종단 short page 는 **관측된 적이 없다**(문서에도 실측에도 없다).
