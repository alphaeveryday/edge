...
MAX_PAGES = 200
# 콜당 응답 상한. **비종단 short page(꽉 안 찼는데 아래에 더 있음)는 실측으로 0건**이다
# (2026-07-26 라이브 프로브 약 400페이지: 12종목 직렬 + 12종목 8워커 동시 각 12페이지, 그리고
# 3종목을 2015년까지 29페이지 깊게 훑음 — 길이가 {100, 37}로 **마지막 1장만 37이고 그 아래를
# 다시 물으면 0행**. 동시 8워커에서도 동일해 부하가 응답을 자르지도 않는다).
# 그래도 창 소진의 **단독 근거로는 쓰지 않는다** — 부재의 관측이 불가능성의 증명은 아니라서,
# 아래 종료조건에서 달력 판정의 교차 검증으로만 쓴다(_stop_bound 주석).
PAGE_SIZE = 100
...
def _stop_bound(d1: str) -> str:
    ...
    ⚠️ **남는 구멍**: 벤더가 **비종단 short page** 를 주고 *동시에* 휴장일 집합이 과잉이면 그
    날 봉을 놓친다. 과잉 등재 하나만으로는 안 생긴다 — 요청 하한이 원본 d1 이라 그 날이 실제로
    거래됐으면 응답에 들어오고, 꽉 찬 페이지에선 달력을 안 믿는다. 그리고 비종단 short page 는
    **약 400페이지 실측에서 0건**이다(PAGE_SIZE 주석). 부재의 관측이 불가능성의 증명은 아니라
    구멍으로 남겨 적되, 두 조건이 겹쳐야 하고 하나는 미관측이라 닫지 않았다.

    이 노출은 **이 최적화가 새로 만든 것이다.** 옛 코드는 short page 여도 `earliest > d1` 이면
    멈추지 않고 그 아래를 다시 요청하므로 비종단 short page 로부터 **회복한다** — `new == 0`
    (진짜 빈 페이지)일 때만 멈춘다. 이 최적화는 그 재요청을 없애 회복 기회를 제거한다.

    ⚠️ **자가 치유는 없다.** 창이 [today-N, today] 로 **앞으로** 밀리므로, 창 최하단에서 놓친
    날은 다음 런에서 창 **밖**으로 나간다. 놓치면 영구 유실이고 collection_log 는 success 다.
    """
