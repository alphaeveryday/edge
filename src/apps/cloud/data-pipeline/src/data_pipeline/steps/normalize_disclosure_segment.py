    ...
    for report_date, new_rows in sorted(by_partition.items()):
        ...
        partitions.append({
            ...
            "winner_ids": [
                ...
                for rcept_no, ordinal in sorted({
                    (row["rcept_no"], row["segment_ordinal"]) for row in current_winners
                })
            ],
        })
...
def run(
    ...
    *,
    ...
    # parser_version staleness(_merge_partition 참고)는 스코프와 무관 — 파서가 바뀌면
    # 어느 쪽 런이든 새 version 행이 들어오고 정리는 후속 재적재 소관이다(변동 없음).
    partitions: list[dict] = []
    canonical_rows = 0
    ...
    # 한 공시 유형의 행 실패가 다른 유형의 성공 manifest를 폐기하지 않는다. 성공 범위를
    # commit하되 반환 코드는 worker가 window를 INCOMPLETE로 남기게 한다.
    ...
    logger.info(
        ...
        len(failures), len(warnings), len(partitions), canonical_rows,
    )
    return exit_code
