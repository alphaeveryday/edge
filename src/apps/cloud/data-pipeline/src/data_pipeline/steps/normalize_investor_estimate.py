...
**`asof_slot` 은 TEXT 로 원문 보존한다.** `bsop_hour_gb` 의 도메인이 미관측이다("0930" 같은
시각인지 "1"~"5" 코드인지). 시각으로 파싱하면 형식을 잘못 가정해 정체성 키를 오염시키고, 소급
재조회가 없는 소스라 사후 정정이 불가하다 — 실측으로 좁혀지기 전까지 원문 그대로 둔다.
