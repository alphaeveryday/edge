...
격자 폭(390)의 배선 반례는 `test_session_cli.py` 에 있다 — 순수 함수(`plan_session_windows`)
...
def test_공시는_소스_단위라_universe_축이_아니다():
    ...
    # 장외 공시는 배치가 회수하므로 상주 격자는 정규장에 머문다.
    assert DATASET_DISCLOSURE_MINUTE not in EXTENDED_HOURS_DATASETS
