...
def test_scale_updates_every_service_with_force(monkeypatch):
    ...
    monkeypatch.setenv(session_ops.ENV_SERVICES, "svc-worker,svc-relay,svc-consumer")
    # `_services` 가 이 env 를 요구한다(ALPHA-912 — 비면 죽는다). 프로덕션 배선과 같은
    # 모양으로 준다: 공용 목록에 없는 이름이라 빼기는 아무것도 안 지운다.
    monkeypatch.setenv(session_ops.ENV_ANALYSIS_SERVICES, "svc-analysis-consumer")
...
class TestAnalysisConsumerOwnList:
    """설명 소비자의 desired 소유 축(ALPHA-910 분리 → **ALPHA-912 이관 완료**).

    ⚠️ 이 클래스의 계약이 **뒤집혔다.** ALPHA-910 에선 "자기 목록으로 오르내리는가"가 계약이었고,
    지금은 **세션이 이 서비스를 아예 안 건드리는가**가 계약이다 — desired 는 큐 잔여 일감을
    보는 오토스케일링이 소유한다. 세션이 하루 한 번이라도 쓰면 그 값이 스케일러의 판정을 덮고,
    특히 20:05 의 0 은 **아직 처리 중인 설명을 자른다**(게이트가 이 큐를 안 본다).

    그래서 여기서 보는 것은 둘이다: 세션의 스케일 호출에 이 이름이 **어느 경로로도** 안 실리는가,
    그리고 공용 목록에서 빼는 근거(자기 목록 env)가 사라지면 **죽는가**.
    """
    ...
    def test_세션은_소비자를_올리지_않는다(self, monkeypatch, wiring):
        ...
        assert all("svc-analysis-consumer" not in (c["services"] or []) for c in wiring.calls), \
            f"세션이 설명 소비자를 스케일했다 — 오토스케일링이 정한 desired 를 덮는다: {wiring.calls}"
        common = [c for c in wiring.calls if c["services"] is None]
        assert common == [{"desiredCount": 1, "forceNewDeployment": True, "services": None}], \
            "공용 목록 호출은 그대로 한 번뿐이어야 한다"
        # 공용 호출이 소비자를 안 싣는 것은 `_services` 의 빼기가 도는 덕이다 — 그 빼기가
        # 실제로 이 이름을 뺐는지 함께 본다(빼기가 죽어도 위 단언은 통과할 수 있다:
        # 공용 목록 env 에 소비자가 없는 픽스처면 아무 일도 안 하고 초록이 된다).
        monkeypatch.setenv(session_ops.ENV_SERVICES, "svc-worker,svc-analysis-consumer")
        assert session_ops._services() == ["svc-worker"]

    def test_자기_목록_env_가_비면_죽는다(self, monkeypatch, wiring):
        """ALPHA-910 의 컷오버 관대함을 회수한 자리다. 빈 값이면 `_services` 의 빼기가 돌
        근거가 없어 **공용 경로가 소비자를 다시 스케일**하고, 그러면 매일 밤 stop 이
        스케일러의 desired 를 0 으로 덮어 축이 도로 둘이 된다. 조용히 되돌아가면 안 된다."""
        monkeypatch.delenv(session_ops.ENV_ANALYSIS_SERVICES, raising=False)

        with pytest.raises(SystemExit, match=session_ops.ENV_ANALYSIS_SERVICES):
            session_ops._services()

    def test_계획이_실패하면_아무것도_안_오른다(self, monkeypatch, wiring):
        """가격 계획이 실패한 날은 스케일 호출 자체가 없다."""
        monkeypatch.setattr(session_ops, "is_trading_day", lambda day: True)
        monkeypatch.setattr(session_ops, "plan_session_cli", lambda *a, **k: 1)
    ...
    def test_세션은_소비자를_내리지_않는다(self, monkeypatch, wiring):
        """20:05 의 0 은 **아직 처리 중인 설명을 자른다** — 게이트가 이 큐를 안 보므로
        (설명 큐는 게이트 밖) 남은 건이 있어도 stop 은 온다. 스케일러는 처리 중(비가시)까지
        세어 그동안 대수를 유지하니, 손을 떼는 것이 곧 그 절단을 없애는 것이다.
        야간 비용은 스케일러가 잔여 0 에서 0 대로 내려 해결한다(실증됨)."""
        monkeypatch.setattr(session_ops, "_queue_depths", lambda queues: [])
        monkeypatch.setattr(session_ops.time, "sleep", lambda _: None)
        ledger = FakeLedger(["DRAINED"])
        ledger.request_drain = lambda *, session_id, now: True
        ...
        assert rc == 0
        assert all("svc-analysis-consumer" not in (c["services"] or []) for c in wiring.calls), \
            f"세션이 설명 소비자를 내렸다 — 처리 중인 설명이 잘린다: {wiring.calls}"

    def test_게이트가_안_비면_아무것도_안_내려간다(self, monkeypatch, wiring):
        """상한 초과에서는 내리지 않는다 — 처리 중인 window 가 결손된다."""
        monkeypatch.setenv(session_ops.ENV_DRAIN_TIMEOUT, "0.01")
...
# (ALPHA-887 의 유예는 ALPHA-911 이 회수했다). 여기 이름이 남아 있다는 것 자체가
# "아직 후속이 안 끝났다"는 표시다.
_CD_ROLL_PENDING_APPLY: set[str] = set()
