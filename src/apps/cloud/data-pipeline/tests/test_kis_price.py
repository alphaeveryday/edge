...
def _source(chunk_responses, *, app_key="k", app_secret="s", symbol_map=None, client=None, concurrency=1):
    ...
    return KisDailyPriceSource(config, client or FakeClient(chunk_responses), concurrency=concurrency)
...
def _multi_symbol_fixture(n: int = 6) -> tuple[dict, dict]:
    """심볼 n개, 각 1페이지 1봉 — 팬아웃 동등성·격리 검증용 최소 픽스처."""
    symbol_map = {f"{i:06d}": f"{i:06d}" for i in range(1, n + 1)}
    responses = {sym: [_ok([_bar("20260724", close=str(70000 + i))])]
                 for i, sym in enumerate(symbol_map)}
    return symbol_map, responses


def test_동시수집_산출물이_직렬과_같다():
    # WHY: 병렬화는 **산출물을 바꾸지 않아야** 한다. 순서까지 같아야 raw ndjson 회귀 비교가
    #      되고, 하류(normalize 파티션 병합)가 수집 타이밍에 흔들리지 않는다. 가격은 심볼당
    #      2콜(페이지네이션)이라 워커가 페이지 순서를 섞을 여지가 KRX 보다 크다.
...
def test_실패격리와_기록순서가_동시성에서도_같다():
    ...
    responses[broken] = ['{"rt_cd": "1", "msg_cd": "OPSQ0001", "msg1": "조회 오류"}']
    src = _source(responses, symbol_map=symbol_map, concurrency=4)
