...
def test_each_board_lands_on_its_own_mic(tmp_path):
    ...
    공식 OpenAPI 는 어댑터가 부른 엔드포인트). 값이 갈리면 같은 종목이 두 market_code 로
...
def test_unknown_board_is_dropped_with_a_reason(tmp_path):
    ...
    rows = [{**_row(f"0{i:05d}", f"코스피{i}"), "board": "KOSPI", "market": "KR",
            ...
            for i in range(10)]
    rows[0] = {**rows[0], "board": "NEXTRADE"}   # 어휘 밖 시장
...
def test_canonical_preserves_the_share_class_verbatim(tmp_path):
