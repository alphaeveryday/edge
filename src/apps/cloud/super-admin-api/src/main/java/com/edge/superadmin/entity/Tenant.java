 ...
 * environment CHECK 는 대문자(PROD/DEV) — UI 표기(Prod/Dev) 변환은 경계(dto·service)에서.
...
public class Tenant {
	...
	public Tenant(String name, String env, String status, OffsetDateTime createdAt) {
		this.name = name;
		this.env = env;
		this.status = status;
	...
	/** 재구성(테스트 시드 등) — id 포함. */
	public Tenant(Long id, String name, String env, String status, OffsetDateTime createdAt) {
		this(name, env, status, createdAt);
		this.id = id;
	}
}
