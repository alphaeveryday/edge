 ...
 * 지금 없는 것: 런 종류({@code kind}) · AWS 제어면({@code aws_status}·{@code queues}) ·
 * 체인 단계 집계 · per-ETF 분석 귀결 원장 · 런북 · 재시도 정책 상한. AWS 축은 C 축(제어면
 * 배선)이 오면 {@code meta.aws}·{@code awsUnavailable}·{@code awsUnobservedRuns} 와 함께 들어온다.
 */
public interface ConsoleFactsRepository {
	 ...
	 *
	 * <p>여섯 조회가 한 스냅샷에서 돌아야 한다 — 런을 읽은 뒤 writer 가 커밋하고 작업을 읽으면
	 * "런은 SUCCEEDED 인데 작업은 PENDING" 같은, 어느 시점에도 존재하지 않은 조합이 한 판정
	 * 위에 조립된다(드릴다운 네 조회와 같은 이유).
	 */
	...
	/**
	 * 런 하나. {@code runKey} 가 곧 사건 식별자의 대상 축이다.
	 *
	 * <p>{@code planned}·{@code noRunRow} 는 <b>런 행이 없는 슬롯</b>에만 채워진다
	 * ({@code ops_reconciliation_issue PLANNER_MISSING}). 실재하는 런 행에는 null 이다 —
	 * "스케줄 상 있어야 할 슬롯인가"를 답하는 계측이 원장에 없기 때문이고(크론 설정은 DB 밖),
	 * 없는 것을 false 로 채우면 모름이 단정으로 바뀐다.
	 */
	 ...
	 * 작업 축으로 나가지 않는다 — 서비스가 <b>데이터셋 축을 파생</b>하는 재료다(계약 §
	 * "{@code dataset_contract} 테이블은 없다": 계약·신선도는 {@code ops_expected_task} 의 컬럼).
	 */
	...
	/**
	 * 산출 하나. {@code base} 는 직전 10거래일 중앙값이고 <b>기준을 만들 거래일이 없으면 null</b>
	 * 이다 — 0 으로 채우면 나눗셈이 성립하는 척해서 "분포 안"이라는 거짓 판정이 선다.
	 */
	record OutputRow(String id, String label, String unit, long today, Double base) {
	}

	/** 게시·발번 경계. 하루가 아니라 <b>누적</b>이다 — 같은 트랜잭션이라는 구조 불변식을 본다. */
