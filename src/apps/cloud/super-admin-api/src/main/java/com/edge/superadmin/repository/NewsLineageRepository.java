 ...
 *
 * <p>{@link Stage} 는 집계 타일 클릭 → 그 부분집합 목록의 드릴다운 축(ALPHA-697)이다.
 * 필터 정의는 집계(summary) 카운트와 <b>같은 SQL 조각</b>이어야 한다 — 갈리면 타일 숫자와
 * 클릭 결과 목록이 서로 다른 집합을 말한다.
 */
public interface NewsLineageRepository {

	/** 문서 목록의 단계 필터 — null 은 전체. 정의는 {@link LineageSummary} 카운트와 동일. */
	enum Stage {
		/** 구조화 증거(assertion) 1건 이상. */
		STRUCTURED,
		/** 증거 0건 — NO_EVENT·추출 실패·미실행이 한 통(구분 미계측). */
		UNSTRUCTURED,
		/** assertion→event_evidence→explanation_run 체인 존재. */
		USED
	}
	...
	List<LineageDocument> documents(LocalDate dateKst, Stage stage, int limit);

	/**
	 * 장중 1분 뉴스 추출 요약(ALPHA-697) — 축은 {@code news_extraction_job.created_at} 의
	 * KST 날짜다. 문서 축(available_at)과 <b>다른 원장</b>이라 같은 날짜라도 두 표의 분모가
	 * 다를 수 있다 — 화면은 이 차이를 명시한다. EOD 레인 실패는 작업 단위로 격자(/sources)
	 * 소관이다(문서 단위 사유는 EOD 원장에 없다).
	 */
	ExtractionSummary extraction(LocalDate dateKst);
	 ...
	 */
	Lineage lineage(LocalDate dateKst, Stage stage, int limit);
	...
	/**
	 * 문서 한 건의 계보 축약 — 존재 → 증거(assertion) → 분석 사용. {@code publisher} 는
	 * 언론사(ALPHA-695 승격, nullable — 게이트가 non-blocking 경고), {@code sourceCode} 는
	 * 수집 벤더다 — 다른 축이라 둘 다 내린다.
	 */
	record LineageDocument(String documentId, String title, String sourceCode,
			String publisher, String sourceUri,
			OffsetDateTime publishedAt, OffsetDateTime availableAt,
			long assertionCount, boolean usedInAnalysis) {
	}

	/**
	 * 1분 추출 job 상태 요약. {@code deadByErrorCode} 는 DEAD 행의 {@code error_code} 별
	 * 건수(건수 내림차순) — 통제 어휘 없는 자유 텍스트라 값 집합은 consumer 분류 소유이고,
	 * NULL(사유 미기록)도 한 행으로 내린다(뭉개면 미기록이 안 보인다).
	 */
	record ExtractionSummary(long succeeded, long dead, List<ErrorCodeCount> deadByErrorCode) {
	}

	record ErrorCodeCount(String errorCode, long count) {
	}
}
