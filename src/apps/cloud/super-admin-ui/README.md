...
| `/grid` | 파이프라인 실행 이력 (데이터셋×날짜 30일 격자, 유형·도메인 필터 — 박스 클릭 시 그날의 실행 목록이 격자 아래에 펼쳐진다. 실시간 세션은 `/minute?date=&dataset=` 로, 배치 실행의 작업 행은 `/ops/runs/:runId?date=` 로 간다 — 실행 상세의 조회 창이 하루라 **그 칸의 날짜를 함께 싣는다**, ALPHA-738) |
