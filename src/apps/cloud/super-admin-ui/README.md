...
| `/grid` | 파이프라인 실행 이력 (데이터셋×날짜 30일 격자, 유형·도메인 필터 — 박스 클릭 시 그날의 실행 목록이 격자 아래에 펼쳐진다. 실시간 세션은 `/minute?date=&dataset=` 로 가고, **배치 실행은 나가는 링크가 없다** — 실행 상세가 스냅샷 런만 해소해서다, ALPHA-738) |
