...
실행이력(`/grid`)의 뉴스 품질 색상은 API의 `qualityAssessment`를 사용한다(ALPHA-1076).
실행 실패·미실행은 장애, 기술 오류·실제 누락은 1건부터 주의다. 성공한 현재 시도에서
원본 해소율과 정책 제외 건수를 보존하며, 새 계측이 있는 실행은 인물·업종·명시된
모호어를 분모에서 뺀 판정 해소율을 사용한다. 과거 실행은 이전 기준으로 표시한다.
부분 계측·모순·판정 분모 0은 미측정이며 기술 오류·실제 누락 판정은 우선한다.
내용상 제외로 설명되는 건만 비율로 판단한다: 엔티티 해소율 **60% 미만**, 주장 제외율
**20% 이상**, 기준 종목 없는 이벤트 비율 **20% 이상**이면 주의다. 주장 제외율의 분모는
적재·기존 주장 수 + 내용상 제외 수, 이벤트 비율의 분모는 이번 실행에서 생성한 이벤트다.
20%는 최근 14일 관측(계측된 이벤트 15회, 최대 약 18%)을 참고한 **초기 운영 상한**이며,
통계적 이상 확률은 아니다. 일별 추이의 60%와 달리 실행이력은 **각 실행**을 판정한다.
기준 이내여도 원장 `INCOMPLETE`와 제외·오류 건수는 상세에 보존한다. 과거 진단에는 주장
제외·기술 오류 분해가 없어 `UNMEASURED`가 될 수 있다. 이때 기존 원장 경고를 유지하고
근거 부족을 표시한다. 새 API 필드가 없는 구 버전 응답도 기존 기준을 유지한다.
...
`datasetCatalog.test.ts` 가 그 다리다(유령·누락·중복·**접는 방향**을 양방향으로 잡는다).
facts-snapshot 을 정본으로 쓰면 안 된다 — 날짜 고정 픽스처라 이후의 레인 이동·신설을 모른다.

⚠️ **원장의 `dataset` 을 화면 행으로 그대로 쓸 수 없다** — 그건 **행 축과 다른 축**이고 더 잘다
(여러 원장 값이 한 데이터셋 행으로 접힌다). 카탈로그가
그것을 수집 데이터셋 한 행으로 **접는 것이 의도**이고, 테스트는 그 접기가 조대화인지(원장 값
하나가 한 행에만 속하는지)를 잰다. ALPHA-981 이 "그 컬럼이면 `taskKeys` 가 불필요해진다"는
전제로 발번됐다가 이 측정으로 무효화됐다.
