  ...
  if (!a) {
    return (
      <div className="p-10 text-center" style={{ color: 'var(--fg-3)', fontSize: 13 }}>
        {/* 🔴 **무엇을 뒤졌는지 밝힌다.** 미리보기에서는 목 배열만 뒤지므로, 실 원장에는
         * 있는 id 로 `?preview=mock` 을 열면(낡은 목 북마크가 그 모양이다) 아래 실 원장 문구가
         * 거짓이 된다 — 실제로는 실 응답을 아예 안 본 것이다. */}
        {preview ? (
          <>이 ID 는 화면 검수용 목데이터에 없습니다 — 실 원장을 조회한 결과가 아닙니다.</>
        ) : (
