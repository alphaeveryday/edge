  ...
  return (
    <nav className="t-xs ops-crumb" aria-label="조사 경로">
      {incidentId ? (
        ...
        <>
          {/* 실행 **목록**이 아니라 원장 근거다 — 아래 404 분기와 같은 이유로, 실 API 런키를
            * 들고 온 사용자를 **조회 창 밖**인 목록으로 보내면 "없다"로 읽힌다
            * (`RUN_DETAIL_UNAVAILABLE` 의 사유와 같다: 스냅샷이라서가 아니라 그 조회가 최근
            * 하루만 싣기 때문이다).
            * ⏭ 이 라벨은 **조사 경로에서 그 화면이 맡는 역할 이름**이고, 도착지의 페이지 제목은
            * `데이터 소스 수집 상태`다(`headerRoute.ts`). 둘이 다른 것은 알고 두는 것이며,
            * 통일하려면 다섯 자리를 함께 고쳐야 한다(여기·:131·:136·:165·RunAxisPage:889). */}
      ...
      {runKey && (
        <>
          {/* 링크가 아니다 — 실행 상세는 **최근 조회일 하루만** 읽어 이 실 API 런을 해소하지
           * 못한다(`RUN_DETAIL_UNAVAILABLE`). ⚠️ "스냅샷만 읽는다"가 아니다: 그 화면은
           * ALPHA-738 D 부터 실 원장(`/console/facts`)을 읽고, 남은 제약은 **창**이다.
      ...
      return (
        <div className="flex flex-col gap-4">
          ...
          <div className="card card-pad">
            ...
            <p className="t-xs m-0" style={{ color: 'var(--fg-3)', marginTop: 4 }}>
              {/* 실행 **목록**으로 보내지 않는다 — 그 화면은 최근 조회일 하루만 실어서, 그 창
                * 밖 런키를 들고 온 사용자가 거기서도 자기 실행을 못 보고 "없다"로 한 번 더
                * 읽는다(스냅샷이라서가 아니다 — `RUN_DETAIL_UNAVAILABLE` 참조).
