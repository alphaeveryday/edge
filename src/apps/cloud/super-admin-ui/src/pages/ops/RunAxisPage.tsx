 ...
 *     넷 다 산다 — 임의 날짜를 다루는 셋은 `?date=` 로 자기 조회 창을 함께 싣는다(축 E).
 *
...
export function RunDetailPage() {
  ...
  /* 이 런이 어느 날의 것인지 **보낸 쪽이 말한다**(축 E) — 조회 창이 하루라, 안 실으면 원장이
   * 아는 가장 최근 날만 보고 임의 날짜의 런은 전부 아래 부재 분기로 떨어진다.
   *
   * ⚠️ **빈 문자열을 부재로 접지 않는다**(`|| undefined` 가 아니라 `?? undefined`). 접으면
   * `?date=` 가 형식 400 을 못 받고 조용히 최신 날로 갈아탄다 — `repository.real.facts` 가
   * 같은 이유로 같은 판별을 쓴다. 모양이 틀린 값은 그대로 보내 서버가 거부하게 두고, 그
   * 거부는 `ConsoleGate` 가 조회 실패로 세운다(화면이 판정하지 않는다).
   * ⚠️ 다만 이건 **손으로 친 주소·북마크에만** 걸린다: 앱이 만드는 주소는 `runHref` 를 거치고
   * 그 함수가 falsy 쿼리를 아예 안 싣는다(위 레거시 `?run_id=` 리다이렉트도 그래서 빈 `date`
   * 를 떨군다). 두 경로가 갈리는 것을 알고 둔다 — 리다이렉트는 링크 생성이 아니라 주소
   * 정규화라 junk 파라미터를 지우는 쪽이 맞다(리뷰가 이 갈림을 짚었다). */
  const date = params.get('date') ?? undefined;

  const ev = useConsoleEvaluation(date);
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {selected ? (
         ...
         *
         * ⚠️ **부재가 두 형상이다**(축 E). 날짜를 안 받았으면 "안 물어본 날의 런일 수 있다"이고,
         * 받았으면 **그 날을 물었는데 없었다** — 뒤쪽에서 앞 문장을 쓰면 보낸 화면이 날짜를
         * 안 실은 것처럼 읽혀, 운영자가 링크 결함을 찾아 나선다. */
        <div className="card card-pad">
          ...
          <p className="t-xs m-0" style={{ color: 'var(--fg-3)', marginTop: 4 }}>
            ...
            이라는 뜻입니다
            {date
              ? '(그 날짜로 물었는데 이 런이 그날에 안 잡혔습니다 — 원장이 이 런을 다른 날로 세었거나 런 키가 다릅니다).'
              : '(다른 날짜의 런일 수 있습니다 — 이 주소는 날짜를 싣지 않아 가장 최근 날만 물었습니다).'}{' '}
            다른 최근 실행을 대신 선택하지 않습니다 — 무관한 런이 이 사건의 실행처럼 보이기
            때문입니다.{' '}
            {incident ? (
