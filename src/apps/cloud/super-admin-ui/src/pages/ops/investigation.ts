 ...
 *
 * ⚠️ **사유가 바뀌었다**(ALPHA-738 D). 실행 상세(`/ops/runs/:runId`)는 이제 앱 번들 안
 * 스냅샷이 아니라 **실 원장**(`/console/facts`)을 읽는다 — 그래서 "실 데이터를 못 읽는다"는
 * 더는 참이 아니다. 남은 제약은 **창**이다: 그 조회는 날짜 인자 없이 돌아 원장이 아는 **가장
 * 최근 날 하루**만 싣는데, 실 API 화면(실행 이력 `/grid` · 현재 실행 `/minute` · 구성종목
 * 결손)은 임의의 날짜를 다룬다. 그 창 밖 런으로 보내면 목적지가 "이 조회 결과에 그 실행이
 * 없습니다"로 끝난다 — 없는 게 아니라 안 물어본 것이다.
 *
 * ⚠️ 문구에 **`거래일` 이라 쓰지 않는다** — 그 하루는 `meta.today`(원장이 아는 가장 최근 날)라
 * 휴장일·계획만 있던 날도 온다. 거래일이라 쓰면 운영자가 조회 축을 거래일 달력으로 읽는다.
 *
 * ⏭ **되살리는 조건은 날짜를 함께 넘기는 것**이다(축 E): `runHref` 가 그 화면의 거래일을
 * 싣고 `RunDetailPage` 가 `useConsoleFacts(date)` 로 그 날을 조회하면 이 상수는 사라진다.
 * 그때 `styles/minute.css` 의 `.mn-runcard` 포커스 링도 같이 되살려야 한다(지금은 링크가
 * 아니라서 지워 뒀고, 이 앱의 `styles/` 에는 전역 `a:focus-visible` 이 없다).
 *
 * 판별 기준은 "상세냐 목록이냐"가 **아니다** — 그 자리에서 사용자가 **특정 런 identity 를
 * 쥐고 있는가**다. 쥔 자리에서 창 밖 런 축으로 내보내면 "이 실행은 없다"로 읽힌다. 안 쥔
 * 자리(조사 문맥이 아직 없는 화면)에서 `/ops/runs` 목록은 정확히 그 용도라 대상이 아니다.
 */
export const RUN_DETAIL_UNAVAILABLE =
  '실행 상세는 최근 조회일 하루만 읽어 이 실행을 열지 못합니다';
 ...
 * 정당한 호출처는 **같은 조회 창 안의 런**을 다루는 자리(사건 조사 경로 · 실행 목록의
 * 선택)뿐이다. 날짜를 함께 싣게 되면(축 E) 그 제약이 없어진다.
 *
