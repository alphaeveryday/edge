...
막다른 길 둘을 먼저 적어 둔다(둘 다 이 문서가 한 번씩 틀리게 주장했던 것이다):

- ❌ **`runs[].deadline` = 완결 시각** — 그건 hard deadline 이라 슬롯보다 한참 뒤다. 동봉 스냅샷의
  `etf-daily:2026-08-03T15:40` 슬롯의 `deadline` 은 **21:40** 이다.
- ❌ **"그 레인의 런이 귀결되면 완결"** — 장에 매인 둘(`o.pub`·`o.trig`)에는 맞지만 **뉴스 갈래
  셋에는 안 맞는다**. `o.doc`·`o.asr`·`o.evt` 는 `available_at` 의 하루 누계이고 `o.asr`·`o.evt` 는
  공시까지 함께 센다 — 오전 런이 다 끝나도 그날 행은 계속 늘어난다. 런 귀결만 보고 판정을 열면
  같은 거짓 P1 이 되돌아온다.

남는 것은 **갈래마다 다른 완결 조건**이다: 장에 매인 둘은 그 레인의 런 귀결, 뉴스 갈래 셋은
그날이 끝나야 한다. 참고로 그 스냅샷의 16:20 시점 etf-daily 런은 `ledger_status: RUNNING` ·
`aws_status: FAILED` 라 **가격 산출도 완결이 아니었다**.
