...
⚠️ **완결 조건은 아직 모른다 — 이 문서가 세 라운드 연속 틀렸다.** 그럴듯한 신호를 적을 때마다
리뷰가 반례를 냈으므로, 네 번째 추측을 적는 대신 **막힌 길만** 남긴다:

| ❌ 후보 | 반례 |
|---|---|
| `runs[].deadline` | hard deadline 이라 슬롯보다 한참 뒤다 — 스냅샷의 `etf-daily:…T15:40` 슬롯 deadline 은 **21:40** |
| "그 레인의 런이 귀결되면 완결" | 뉴스 갈래 셋(`o.doc`·`o.asr`·`o.evt`)은 `available_at` 하루 누계라 런이 끝나도 행이 는다(`o.asr`·`o.evt` 는 공시까지 함께 센다) |
| "장에 매인 둘만이라도 런 귀결로" | `o.pub` 은 etf-daily 가 안 쓴다 — `explanation_result` 를 **상주 analysis 소비자**가 쓴다. 런이 SUCCEEDED 여도 큐가 남아 있으면 계속 는다. 런 귀결이 성립하는 건 `o.trig`(etf-daily 의 `LoadPriceTriggers`) 하나뿐이다 |

**그 조각의 첫 일은 산출 다섯의 생산 경로를 하나씩 짚는 것이다** — 완결 신호는 산출마다 다르고,
`OutputSpec` 의 `marketBound` 로는 안 갈린다. 참고로 스냅샷의 16:20 시점 etf-daily 런은
`ledger_status: RUNNING` · `aws_status: FAILED` 였다(그때는 `o.trig` 조차 완결이 아니었다).
