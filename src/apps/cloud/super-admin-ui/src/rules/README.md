...
읽는데, 산출 다섯은 완결 시점이 제각각이다(무엇이 완결을 정하는지는 아래 참조 — 세 라운드 동안
못 맞혔다). 그래서 낮에 조회하면 **아직 안 만들어진 것이 −100% 로 선다**.

실측:

| 조회 | 산출 | today | base | 판정 |
|---|---|---|---|---|
| dev 2026-08-11 14:27 KST | `o.trig` 배치 트리거 | 0 | 28.5 | −100% P1 (거짓 — etf-daily 15:40 슬롯 전) |
| 〃 | `o.pub` 게시 ETF | 8 | 27.5 | −71% (설명 생산이 아직 진행 중) |
...
방향(미착지) — 규칙 층이 `ctx.now` 와 **산출별 완결 여부**를 함께 봐야 한다. 그런데
⚠️ **그 "완결 여부"를 무엇으로 재는지가 아직 모르는 부분이고, 이 문서가 세 라운드 연속 틀렸다.** 그럴듯한 신호를 적을 때마다
