...
| `rules.test.ts` | `pnpm test` — 규칙당 위반/비위반 픽스처 + 경계 케이스 (node:test) |
...
⚠️ **회귀 기준 29/20/6 은 이 5건을 포함한 값이다.** 고치면 위반 29→24 · 사건 20→15 로 움직인다 —
기준을 옮기는 것이 그 조각의 본문이다.
...
방향(미착지) — 규칙 층이 `ctx.now` 와 **산출별 완결 시각**을 함께 본다. 갈래마다 다르다:
장에 매인 둘은 그 레인의 마감(이미 사실에 있다 — `runs[].deadline` 의 `etf-daily:…T15:40`),
뉴스 갈래 셋은 **그날 끝**. 없는 것은 **산출↔완결 시각 대응**뿐이고 R13 하나만 쓰는 선언이라
이 파일의 작은 표로 족하다. 그때 R13 은 완결된 산출만 판정하고 나머지는 `note` 로 밝힌다
(R03·R10·R19 의 부분 관측 표기와 같은 형태).

대안(더 무딤): 대응표 없이 **당일 조회의 음(−) 편차를 통째로 미룬다**. 짧지만 같은 날의 진짜
감소까지 삼키고, 완결된 뒤에도 미룬다 — 스냅샷(16:20 KST, 가격 마감 후)의 `o.trig` −37% 가
그렇게 사라진다.
