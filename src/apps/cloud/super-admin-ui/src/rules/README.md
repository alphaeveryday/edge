...
5. **실행 위치** — 명세는 "서버 쪽이 자연스럽다" 했으나 UI 워크스페이스의 순수 TS 모듈 + node CLI 로 구현했다.
   ...
   ⚠️ 둘을 합쳐도 **19규칙이 다 서지는 않는다** — `/console/facts` 에 큐 구독 매핑·
   재시도 상한이 없어 R11·R16 은 계속 `evaluated:false` 다. 어댑터 둘을
...
🔴 **상시 계측 공백은 셋(R07·R11·R16)이다.** 화면은 그걸
"못 돎"으로 정직하게 그리지만, 운영자에게는 **그 축을 아무도 안 보고 있다**는 뜻이다.
✅ R10·R03·R12는 ALPHA-979 **조각 1·2·3**이, R15는 ALPHA-978이 실축을 배선하며 빠졌다.
✅ R08도 ALPHA-978의 KIS NAV `actual_as_of` writer가 배선했다. 단, 조회 응답에 검증 가능한
NAV 기준일이 없으면 그날은 조건부로 `evaluated:false`를 유지한다.
