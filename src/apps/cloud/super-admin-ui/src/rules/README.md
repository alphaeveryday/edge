...
6. **구 홈 병존** — `/`는 규칙 엔진 사건 목록(`/ops/incidents`)으로 이동하고, 이전
   `OverviewPage`(실데이터 레인 요약)는 `/overview`에 남는다. 차이는 출처가 아니라 **묻는 질문**이다:
   규칙 엔진은 "무엇이 걸렸는가"를, 레인 요약은 "레인이 지금 어디까지 왔는가"를 답한다.
