...
test('스냅샷 회귀 — 동봉 스냅샷은 위반 24 · 사건 15 · P0 6 (레퍼런스 v4 대비 R14 강등 제거분 +1)', async () => {
  ...
  /* 사건 P0 는 **안 움직인다** — R13 은 P1 이다. 위반·사건만 줄고 P0 가 그대로인 것이
   * "줄어든 것이 R13 뿐"이라는 증거다(다른 규칙이 함께 죽었으면 여기가 먼저 깨진다). */
  ...
  /* ⚠️ **줄어든 자리를 이름으로 못박는다.** 수만 고정하면 "R13 다섯이 빠지고 다른 다섯이
   * 새로 걸린" 상태도 24/15 로 통과한다. R13 은 위반이 아니라 **못 돎**이어야 하고(기준이
   * 없으면 "분포 안"이 아니라 분포를 모른다), 그 사유가 화면에 실려야 한다. */
