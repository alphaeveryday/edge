 ...
 * 규칙이 알아야 한다**(`MinuteFacts.deadJobsByDate`): 모르면 날짜 축 집계를 벤더마다 복제해 같은 사실을
...
export interface MinuteFacts {
  ...
  /**
   * `deadJobs` 가 **세션 축이 아니라 `(dataset, date)` 집계**인 데이터셋들 — 벤더로 못 가른다.
   *
   * 뉴스 job 은 세션 연결 컬럼이 없어 날짜 축 집계 하나뿐이다. 그걸 세션마다 실으면 벤더가
   * 둘인 날 **같은 사실이 벤더 수만큼 복제되어** 독립 사건이 된다(DEAD 3건이 6건으로 읽힌다).
   * 규칙은 이 집합을 보고 대상을 데이터셋으로 낸다 — 값의 입도와 사건 식별자의 입도를 맞춘다.
   *
   * **세션 필드가 아니라 여기 있는 이유**: 축은 데이터셋의 성질이지 세션의 성질이 아니다.
   * 세션마다 두면 같은 데이터셋의 두 세션이 서로 다른 축을 갖는 상태가 **표현 가능**해지고,
   * 그게 안 일어난다는 보증이 생산자(어댑터)의 습관에 걸린다. 여기 두면 표현 불가가 된다.
   *
   * ⚠️ **옵셔널이 아니다.** 생략을 허용하면 소비자가 `?? []` 로 접고, 그 순간 "생산자가 안
   * 말했다"와 "날짜 축 데이터셋이 없다"가 구분 불가가 된다 — 이 콘솔이 없애려는 부재/0 혼동
   * 그 자체다. 그리고 축을 세션에서 여기로 올리면서 **폭발 반경이 커졌다**: 예전엔 세션 하나가
   * 축을 빠뜨렸지만, 지금은 이 필드 하나가 빠지면 모든 데이터셋이 한꺼번에 세션 축으로
   * 되돌아가 뉴스 DEAD 가 벤더 수만큼 복제된다. 필수로 두면 그 확률이 0 이다.
   * `[]` 는 거짓 0 이 아니라 실측이다 — 생산자가 데이터셋 전부를 보고 판정한다.
   */
  deadJobsByDate: string[];
}
