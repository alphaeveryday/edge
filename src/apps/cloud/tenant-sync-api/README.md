...
`controller`(HTTP 검증·공통 응답 포맷) → `service`(SyncBundleService 오케스트레이션) → `repository`(TenantDeliveryRepository — `tenant_delivery` ⋈ 경계면 테이블 JPQL 프로젝션 + BundleEntryStore — 와이어 매핑·delivery_type 분기) / `entity`(`@Immutable` 부분 매핑 — `ddl-auto=validate`, 스키마는 Flyway SSOT, ADR-0038) / `dto`(계약 와이어 포맷 레코드 — DB 엔티티 아님) / `tenant`(보안 횡단 — TenantResolver). **번들 조립은 이 모듈이 경계면 테이블을 직접 조회해 수행한다(ADR-0026) — 외부에서 만들어진 번들을 받지 않는다.** 이 모듈의 DB 접근은 **읽기 전용**이다(outbox writer 는 analysis-engine write-time fan-out — ALPHA-493) — 리포지토리는 `Repository` 마커 상속으로 쓰기 표면을 봉인한다.
`evidences` 는 lineage 두 갈래(이벤트 근거·공시 정규화 사실) 배치 조회(`TenantDeliveryRepository.findEvidenceRows`)로 조립돼 실린다(ALPHA-718 — 경계면 컬럼 선별은 ALPHA-395). `source_events` 는 온프렘 소비자가 없어(이벤트 타임라인 UI 이연) 빈 배열로 실린다 — 소비 UI 도입 시 같은 방식으로 조립한다.
