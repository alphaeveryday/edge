...
/**
 * Cloud 스키마 위에서 실 Postgres 로 검증하는 통합 테스트의 공통 베이스(ALPHA-572).
 * 스키마 SSOT 는 Flyway(libs/schema)이므로 컨테이너에 migrations-cloud 를 적용한 뒤
 * 그 위에서 조회 계약을 확인한다(ADR-0005). H2 는 IDENTITY·TIMESTAMPTZ 등 Postgres
 * 문법을 대체하지 못해 이 검증을 대신할 수 없다. super-admin-api 의 동명 베이스와
 * 같은 패턴(ALPHA-526)이다.
 *
...
public abstract class CloudPostgresIntegrationTest {
