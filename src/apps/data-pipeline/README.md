...
# 판정에 쓴 가격 coverage 는 평가한 전 거래일치가 quality_log(coverage_by_date·coverage_min)에,
# 생성된 행은 detection_reason 끝의 |coverage=… 에 남는다(ALPHA-452 — 1% 비중 종목 하나로
# 판정된 트리거를 사후에 구분하기 위함). 하한으로 막지는 않는다(ALPHA-453).
