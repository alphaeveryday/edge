...
public interface PublicationRepository extends Repository<Publication, Long> {
	 ...
	 */
	@Query(SERVE_JPQL + " ORDER BY p.tradeDate DESC, p.publishedAt DESC")
	Optional<Publication> findLatestPublished(@Param("ticker") String ticker, Limit limit);

	/** 특정 거래일의 게시분 — 같은 거래일에 재게시가 있으면 최근 게시 시각이 이긴다. */
	@Query(SERVE_JPQL + " AND p.tradeDate = :tradeDate ORDER BY p.publishedAt DESC")
