 ...
 * 좁은 Repository 로 save 와 항목별 read 만 노출한다(append-only — 갱신·삭제 없음).
 */
public interface AnalysisItemStatusHistoryRepository
	...
	/** 항목 상태 변경 이력(오래된 순) — 검수 상세의 이력 절 원천(ALPHA-436, 구 439 흡수). */
	java.util.List<AnalysisItemStatusHistoryEntity> findByAnalysisItemIdOrderByStatusHistoryIdAsc(
