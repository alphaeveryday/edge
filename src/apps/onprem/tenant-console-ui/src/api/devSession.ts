 ...
 * 세션 쿠키를 확보한다. main.tsx 가 dev 이거나 데모 박스 빌드(VITE_DEMO_AUTOSESSION='true')
 * 일 때만 동적 import 하므로, 실 온프렘 빌드(플래그 없음)에는 이 파일(자격증명 포함)이
 * 실리지 않는다 — 로그인 화면(ALPHA-486/423) 도입 시 이 브릿지·플래그를 제거한다.
 */
