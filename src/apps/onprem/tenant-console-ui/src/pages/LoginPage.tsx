/* 로그인 화면 (ALPHA-626, super-admin ALPHA-616 패턴 이식).
 * ConsoleLayout 밖 공개 라우트라 .edge-root(폰트·토큰 스코프)를 스스로 감싼다.
 ...
 * 테넌트 브랜딩(tenantMark·tenantName)은 세션 이후에만 아는 값이라 여기서는 EDGE 마크만 쓴다.
 * OTP·인증완료 뷰는 범위 밖(서버 2FA 미지원 — ALPHA-474 계열 후속). */
...
export function LoginPage() {
  ...
  // 쿼리스트링·해시까지 보존 — 상세 딥링크 복귀 시 컨텍스트를 잃지 않게.
      ...
      >
        ...
        <div className="flex flex-col gap-4" style={{ maxWidth: 460 }}>
          ...
          <span style={{ color: 'var(--gray-400)', fontSize: 13, lineHeight: 1.7 }}>
            가격 변동 설명 검수, 컴플라이언스 정책, 노출 범위를 사내 환경 안에서 관리합니다.
          </span>
        </div>
        <div />
      </div>
