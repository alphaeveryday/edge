...
## 문서 ERD 그림 규약 (`docs/data-model/`)

그 대조를 받는 그림 쪽 규약이다. 그림은 두 갈래로 만들어져 있다 — 손으로 쓴 SVG(도메인 5장 +
overview)와 draw.io CLI export(`documents`·`market`·`onprem`). export 를 갱신할 때 지킬 것:

**컬럼 행 라벨은 평문(`html=0`)으로 둔다.** HTML 표 라벨은 export 에 래스터 폴백 `<image>` 를 함께
낳는다(온프렘 실측 41KB → 712KB, 그중 621KB 가 base64 PNG). markdown 에 박힌 SVG 는
`foreignObject` 를 렌더하지 않으므로 **그 래스터가 실제로 보이는 층**이 된다 — 확대하면 흐려지고
문자열 검색도 안 된다.

**배경은 흰색으로 통일한다.** draw.io CLI 는 투명 배경 + `color-scheme: light dark` 를 내므로 그대로
커밋하면 GitHub 다크 테마에서 배경이 검게 깔린다(그림이 보는 사람 테마에 따라 달라진다). drawio
파일의 `background` 속성은 SVG export 에 실리지 않으니(실측) export 뒤에 전면 흰 배경을 넣는다:
...
손으로 쓴 SVG 는 같은 일을 `.canvas` 클래스(`fill:#ffffff`)로 한다. `validate-doc-erd.py` 가 둘
(다크 분기 없음·뷰박스를 덮는 흰 rect)을 검사하므로 빼먹으면 `data-model-validate` 가 빨간불이다.
...
셀로 낸다. **이 축만 가드가 없다**(두 형태를 한 규칙으로 재기 어렵다) — 새 ERD 를 넣을 때 직접
확인해야 한다.

**edge 규약**: 검증기가 보는 id 는 `relation-<N>` 과 그 라벨 `relation-<N>-child`·`-parent` 다.
draw.io 앱에서 선·라벨을 새로 그리면 랜덤 id 가 붙어 대조에서 빠진다(그림엔 보이는데 게이트는
없다고 판단한다) — 박스 위치·크기는 자유롭게 옮겨도 되지만 선과 라벨은 지우고 다시 그리지 않는다.
