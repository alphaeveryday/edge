...
2. 동기화 경로(sync-agent→intake→screening-worker)가 게시 데이터를 적재할 때까지 대기 후, 200 확인:
   ...
   **200 이 나와야 측정 의미가 있다** — 404 는 `PUBLICATION_KNOWN_TICKERS` 와 종목 불일치. (헤더 불필요 — ADR-0053 으로 고객 식별·채널 헤더 폐지, 200 쓰기는 serving_request_metric 1건.)
...
3. 단계마다 기록: `http_req_duration` p95·p99, `http_reqs`(실제 처리율), `http_req_failed`, `checks`(k6 출력값은 **성공 비율** — 100% 미만이면 기대 응답(200/204) 밖이 있었다는 뜻이고 threshold 가 실행을 실패로 승격), `dropped_iterations`(도착률을 못 따라간 수 — **포화 신호**), `ok_responses`(200 응답 수).
